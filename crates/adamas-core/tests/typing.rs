...
/// Пока нет индуктивных типов, **`Type 0` необитаем**: замкнутый терм этого
/// типа построить не из чего - `Pi` живёт в `max` уровней своих частей, и
/// опуститься до нуля неоткуда. Поэтому там, где нужен обитаемый тип, берётся
/// `Type 1`, а его жителем служит `Type 0`.
fn ty1() -> Term {
    Term::universe(1)
}
