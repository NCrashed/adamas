...
//! FBIP-цикл, SIMD-ядро. Столбцов у таблицы два: понижение через C против
//! соседа на Rust (§6) и **LLVM против C** (предпосылка самой фазы). Таблица —
...
//! три нагрузки, их соседи и второй столбец у двух из трёх — колонное ядро
//! LLVM-эмиттер не берёт, и это утверждение прогона, а не примечание.
//!
//! # Второй столбец: что уравнено, чтобы число мерило бэкенд
//!
//! Сборочные пути двух бэкендов различаются по построению, а Фаза 6 намерила
//! разрыв, 99% которого оказались строкой сборки. Что уравнено и чем это
//! предъявлено — в шапке [`harness`], разделом «LLVM-путь»; здесь только
//! следствия для стенда.
//!
//! Обе стороны каждой нагрузки выходят из **одного** специализированного терма
//! ([`Both`]) и сверяются между собой ответом и счётчиком блоков ещё до
//! замера ([`harness::same_work`]). Свидетелей включает
//! [`harness::WITNESS`]; они печатают, что зовёт горячая функция каждой
//! стороны, сколько пар `dup`/`drop` снимает проход после инлайнинга, и мерят
//! пять дополнительных отношений — два замедления одной стороны и три цены
//! строки сборки.
//!
