...
enum Why {
    ...
    Regional,
    /// Вектор (§4.9): плоский, но укладки у него этот срез не считает.
    Vectorial,
...
impl Walk<'_> {
    ...
    fn layout(&mut self, metas: &Metas, ty: &Rc<Value>) -> Result<Layout, Blame> {
        ...
        match &*value {
            ...
            // Вектор (§4.9) плоский по существу - заголовка у него нет, - но
            // укладку ему здесь не выдают: воспользоваться ею мог бы только
            // `Array n (Simd w a)` либо запись с векторным полем, а ни того, ни
            // другого трек H не берёт. Названный отказ честнее выданного числа,
            // которым никто не пользуется.
            Value::Neutral(Head::Simd, _) => Err(Blame::alone(
                Shown::Named(Rc::from(adamas_core::prim::SIMD)),
                Why::Vectorial,
            )),
