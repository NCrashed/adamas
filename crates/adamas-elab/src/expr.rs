...
///
/// Сопоставляются **объявленные** имена, а не написанные: операция эффекта из
/// тела модуля объявлена квалифицированной, а ветка для неё пишется коротким
/// именем - тем же, каким операция и зовётся (§4.8). `declared[i]` - то, во
/// что разрешилось имя `branches[i]`.
fn ordered_branches<'a>(
    ...
    for (branch, name) in branches.iter().zip(declared) {
        ...
        seen.push(name);
        let known = &**name == RETURN || operations.contains(name);
        if !known {
            return Err(ElabError::HandlerBranch {
                name: Rc::clone(&branch.name.text),
...
/// литерал одним узлом или цепочкой.
///
/// **Откуда число.** Замер в наименьшем окружении языка - порождённый поток с
/// умолчательным стеком в два мегабайта, отладочная сборка. Одинокий унарный
/// литерал роняет там понижение на **135**; 134 проходит. Предел взят 112, то
/// есть 83% от измеренного.
///
/// **Прежнее число (256) было больше того, что язык выдерживает**, и почти
/// вдвое: `big : Nat = 256` убивает двухмегабайтный поток на ровном месте. Оно
/// было выбрано «той же величиной, что у предела вложенности разбора», то есть
/// по симметрии записи, а не замером.
///
/// **Чего он не закрывает, и это названо числом.** Глубина написанного вокруг
/// литерала складывается с его собственной: под 64 звеньями - предел разбора -
/// проходит унарный 65 и роняет 66. То есть `f (f (… (112)))` роняет процесс,
/// не нарушив ни одного предела. Опустить 112 до 65 нельзя: корпусу нужен
/// **100** (`tests/golden/eval/unwind-inner-handler.adamas`, `plus 100 n`), а
/// 64 + 100 в два мегабайта не помещается ни при каком делении бюджета.
/// Значит, суммой (а) не закрывается вовсе, и закрывает её только узел ядра
/// под литеральный буфер (б) либо нерекурсивные обходы (в) - §10 вопрос 187.
pub(crate) const UNARY_LIMIT: u32 = 112;
...
impl<'a> Elaborator<'a> {
    ...
    fn masked_label(&self, inner: &Expr) -> Option<Symbol> {
        ...
        // Голова вправе быть путём: операция эффекта из тела модуля снаружи
        // иначе не называется (§4.8).
        let performed = crate::own::head_path(inner)
            .map(|head| self.declared_name(&head))
    ...
    fn masking(
        ...
        // Хвост закрывается: он приходит от подъёма и инстанцируется **при
        // вызове** (§3.4), поэтому меток из него под маской брать нечего. Пока
        // он оставался открытым, недостающее вхождение решалось им самим -
        // `sneaky : {Ask} Nat = mask ask` проходил и уводил операцию мимо
        // хендлера, то есть определение считало не то, что говорит его
        // сигнатура (ревью 2026-09-07).
        //
        // Названная цена: отказ приходит из финальных ворот и говорит про
        // форму приостановленного вычисления, а не про недостающую метку.
        // Сказать точнее отсюда нечем - элаборация аргумента проходит, и
        // расхождение всплывает позже (замерено).
        let rho = Row::closing(rho.labels().to_vec(), None);
    ...
    /// Кратность суждения `ω`, а не `0`, и довод тот же, что у `run`: при `0`
    /// действует стёртый фрагмент, а с ним - §4.7. Спросить тип нетотального
    /// определения при `0` нельзя вовсе («не тотальна и не может стоять в
    /// типе»), тогда как **спросить тип** и **поставить в тип** - разные
    /// вещи. Расходилось это на рекурсивном разборе: `handle parseTerm with`
    /// над членом `mutual`, чей вердикт отрицателен, отвечал «тип написанного
    /// не выводится» (§10 вопрос 112). Настоящее суждение придёт позже, со
    /// своей кратностью.
    ...
    fn name(&mut self, name: &ast::Name) -> Result<Term, ElabError> {
        ...
        if let Some((op, prim)) = prim::PrimCmp::named(&name.text) {
            return Ok(Term::Prim(Prim::Cmp(op, prim)));
        }
    ...
    /// написанному типу связывания (§10 вопрос 105); с ресурсом тип всё равно
    ...
    fn application(&mut self, expr: &Expr) -> Result<Term, ElabError> {
        ...
        // Ожидаемый результат снимается **до** головы: её элаборация им ходит.
        // Лямбда-голова шагает по нему под своё связывание и назад не кладёт,
        ...
        //
        // Источников у типа три, и второй с третьим появились здесь: без них
        // **голый литерал** аргументом терял ожидание и разворачивался унарно,
        // жалуясь на ненайденное `Zero`. Две формы, обе замерены на 698b019 -
        // то есть до литеральных паттернов, дефект пришёл с примитивами:
        //
        // - `g n = g (subUInt64 n 1)` при `g 5` - `σ = 0` отвергает
        //   **нетотальное** имя (§4.7: стёртый фрагмент его не принимает), а
        //   всякий цикл нетотален по построению;
        // - `bounce 0 = bounce 5` - имени нет **в сигнатуре**: пока группа
        //   объявляется, её там нет вовсе (§10 вопрос 50).
        //
        // Первое лечит второе `σ`, второе - тип от самой группы, и оба даёт
        // `synthesized`: спросить тип и поставить в тип - разные вещи, и
        // спайн он снимает по связыванию за аргумент, поэтому уже вставленные
        // головой имплиситы считаются, а не задваиваются.
        //
        // Порядок «`0`, потом остальное» сохранён: при `ω` считается расход, и
        // стёртая позиция отвергается кратностью. Ответ здесь best-effort -
        // сказать, какое `σ` верно, дело настоящей проверки.
        ...
        //
        // Источников у него два, и второй нужен ровно тогда, когда первого
        // нет. `infer` не выводит тип лямбды: `Term::Lam` домена не хранит -
        // его знает проверяющий из ожидаемого `Pi`. Поэтому у применения с
        // лямбдой в голове тип не выводился никогда, умолчание считало
        // результат функцией, и `(\b -> closeFile h) True` отвергался как
        // побег - при том что ничего не возвращается, а §3.3 такое применение
        // прямо разрешает (§10 вопрос 100). Ожидаемый результат тут известен
        // всегда, когда правило и может сработать: живёт он в хвостовой
        // позиции, а хвостовая позиция и есть то, что правило запрещает.
        //
        // Консервативность при этом не ослабляется: `(\b -> keep h) True` при
        // ожидаемом `Bool -> …` остаётся замыканием, потому что ожидаемое -
        // `Pi`, и свойство переживает применение.
        let known = ty.as_deref().or(awaited.as_deref());
        let closure = !known.is_some_and(saturated);
        // Недоприменённое есть замыкание и над **уже поданными аргументами**, а
        // не над одной головой. Пока свойство снималось только с головы, вопрос
        // 90 был закрыт наполовину: `feedPut k s = k MkUnit` при линейном `k`
        // отвергался, а зеркальное `escaping h = keep h` при
        // `keep : (1 h : File) -> Bool -> Bool` проходило и возвращало наружу
        // ω-замыкание над линейным `h`; двойное закрытие на нём исполнялось
        // (лог 2026-09-05). Предикат тот же, каким лямбда считает свой захват:
        // `keep h` и `\b -> keep h b` - одно значение, и различать их правилу
        // незачем.
        //
        // Спрашивается это **только у недоприменённого**: у насыщенного
        // свойство снимается всё равно, а проход по написанным аргументам
        // стоит скана scope на каждое применение - слишком дорого для формы,
        // которой большинство применений и является.
        self.produced = closure
            .then(|| {
                captured.or_else(|| {
                    arguments
                        .iter()
                        .rev()
                        .find_map(|it| self.captured(it))
                        .cloned()
                })
            })
            .flatten();
    ...
    fn stepped(&mut self, ty: &Rc<Value>, argument: &Term) -> Option<Rc<Value>> {
        ...
        // Не прошли ворота - кодомен спрашивается у **нейтрали**, а не теряется.
        //
        // Терять его было дороже всего в диагностике: тип результата уходил в
        // `None`, применение по умолчанию считалось недоприменённым, и отказ
        // выходил наружу как «привязано к своему scope» - про `resume`, который
        // ни при чём. Настоящая причина (аргумент не гасится окружающей) при
        // этом не показывалась вовсе, потому что до настоящей проверки дело не
        // доходило.
        //
        // Нейтраль здесь верна ровно в той мере, в какой этот тип и нужен: у
        // него спрашивают одно - осталась ли стрелка. Приём тот же, каким
        // кодомен снимается в `peeled`.
        //
        // Названный остаток: кодомен, у которого **сама стрельчатость** зависит
        // от значения аргумента, под нейтралью застревает, а застрявшее
        // считается насыщенным. Сойтись это должно с тремя условиями разом -
        // ворота ошиблись, кодомен вычисляет стрелку по значению, и значение
        // привязано к scope, - и до тех пор отказ, показывающий не ту причину,
        // обходится дороже.
        if checked.is_err() {
            // Дырка, а не переменная текущей глубины: `Lvl(ctx.size())`
            // адресует связывание, которого нет - адресуемы `0..size-1`, - и
            // тип, унёсший её в себе, роняет процесс, когда диагностика
            // читает его обратно. `MkSome Type Bool` на семи строках давал
            // «уровень 0 вне контекста размера 0» (ревью 2026-09-07); уровень
            // при этом всегда ровно `ctx.size()`, замерено.
            //
            // Дырка замкнута по построению и на единственный вопрос, который
            // этому типу задают - осталась ли стрелка, - отвечает так же.
            let unknown = self.hole();
            return Some(codomain.apply(unknown));
        }
    ...
    fn snatched(&mut self, computation: &Expr, effect: &Symbol) -> Result<Handling, ElabError> {
        ...
        let ty = match &awaited {
            ...
            // Обход назван двумя половинами, и вторая существенна. Одного
    ...
    fn handled(&mut self, handled: Handled<'_>) -> Result<Term, ElabError> {
        ...
        let written: Vec<Symbol> = branches
            .iter()
    ...
    /// Имя, под которым объявлено написанное: лестница §4.8, а не найдётся -
    /// само написанное.
    ...
    fn statements(&mut self, stmts: &[Stmt], position: Position) -> Result<Term, ElabError> {
        ...
        match &first.kind {
            ...
            //
            // Правило исполнения (§3.4) стоит на хвосте **здесь**, до обёртки
            // `#closing`: вставка деструктора заворачивает остаток блока, и
            // решение, принятое снаружи обёртки, либо не видело хвоста вовсе
            // (признак приостановленности внутрь применения не смотрит), либо
            // исполняло бы его эффекты уже за пределами scope - после
    ...
    fn bindings(
        ...
        // исполнении по объемлющему, хендлеру хвоста - ответ объемлющего;
        // протёкшая аннотация ломала обоих последних (§10 вопрос 131).
        self.result = outer;
        // §3.3: связывание, инициализированное привязанным к scope значением,
    ...
    fn constructor_variables(
        ...
        for (position, field) in fields.iter().enumerate() {
            ...
            match (codomain, value) {
                ...
                // Телескоп дальше не шагает, но **связывать поля надо всё
                // равно**: сколько их, говорит паттерн, а не телескоп. Ранний
                // выход отсюда терял все поля после первого, и `case` по
                // конструктору параметризованного семейства связывал одно -
                // тип разбираемого в ветвь не приходит, поэтому `known` там
                // ложно с самого начала (§10 вопрос 110). Ошибка была видна
                // только когда поля разных типов: у одинаковых пропажа
                // незаметна, а `Pair` без параметра шёл другой веткой.
                _ => known = false,
            }
        }
        ...
        // Значение возвращается только пройденным телескопом: недошагавший
        // спайн вызывающему не годится - шагать по нему свой он не сможет.
        if !known {
            return None;
        }
