...
//! [`Unique`](crate::ir::Unique). §5.1 говорит, что линейное значение обходится
//! без RC, «компилятор знает уникальность статически», - на этом уровне не
//! выполняется (§10 вопрос 149). Кратность ограничивает употребление
//! вызываемым, а не алиасинг: `swap : (1 t : Two) -> Two`, позванная от
//! ω-значения, законна, и объект внутри неё разделён. Замер -
//! `tests/perceus.rs`, одна и та же `swap` стоит лишнюю ячейку.
