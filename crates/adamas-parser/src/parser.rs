...
pub enum ParseError {
    ...
    /// предела достаточно тысячи скобок, чтобы компилятор упал вместо
    /// сообщения. Урок Фазы 0 (`adamas-warmup-stlc`), перенесённый сюда.
    ...
    /// Аргументов в одном применении больше предела.
    ///
    /// Спайн набирается циклом, поэтому разбор его переживает, а вот всякий
    /// рекурсивный проход по получившемуся дереву - нет (§10 вопрос 62).
    #[error("аргументов в применении больше предела в {limit}")]
    TooLong {
        /// Предел.
        limit: u32,
        /// Где кончилось терпение.
...
pub fn parse(text: &str, tokens: &[Token]) -> Result<Module, ParseError> {
    ...
    Parser::new(text, tokens).module()
}

/// Предел вложенности.
///
/// Считает не уровни исходника, а входы в [`Parser::nested`]: скобка стоит
/// двух (`atom`, затем `expr`), стрелка, лямбда, `if`, `case`, блок и
/// вложенное объявление - по одному. Через один из этих входов проходит каждый
/// цикл рекурсии, поэтому предел ограничивает стек целиком.
///
/// Значение то же, что в warm-up'е Фазы 0: 256 входов - это больше сотни
/// вложенных скобок, чего в написанном человеком коде не бывает, и заведомо
/// меньше глубины, на которой кончается стек.
///
/// Считать в скобках при этом обманчиво: стрелка правоассоциативна и тратит
/// вход на каждое звено, поэтому потолок в стрелках - около 250 в одной
/// сигнатуре. Руками столько не пишут; если упрётся сгенерированный код,
/// снимать предел не нужно - достаточно собирать стрелки циклом и сворачивать
/// вправо в конце.
const MAX_DEPTH: u32 = 256;

/// Предел числа аргументов в одном применении.
///
/// Вложенность разбор ограничивает, а длину спайна - нет: `f a b c …`
/// набирается циклом и стоит одного входа. Дерево от этого получается плоским,
/// но глубоким для всякого, кто идёт по нему рекурсией, - и таких большинство:
/// `infer_app` и `eval` в ядре, будущий codegen, наконец `Drop` у самой
/// цепочки `Box`. Предел стоит здесь, потому что здесь он один на всех
/// потребителей сразу (§10 вопрос 62, вариант (а)).
///
/// Замер: в debug `check` срывается между 1200 и 1300 аргументами. Предел взят
/// впятеро меньшим - стек на кадр зависит от того, что на нём уже лежит, и
/// граница сдвигается от вложенности вокруг применения. Цена названа: спайн
/// длиннее 256 не напишет и сгенерированный код.
const MAX_SPINE: u32 = 256;
