...
static adamas_evidence *closing_evidence(adamas_frame *frame, adamas_frame *rest) {
    ...
    for (adamas_frame *below = rest; below != NULL; below = below->below) {
         ...
         *
         * **Свидетель у половины про питомник появился** треком I волны 2 Фазы
         * 7: `adamas-codegen/tests/nursery.rs`,
         * `a_destructor_of_an_abandoned_task_may_yield`. Программа уступает из
         * деструктора брошенной задачи, кругов у неё два - внутренний брошен,
         * внешний жив, - и след `1 7 4 9` без подавления становится `1 7 9 4`.
         *
         * Прежняя запись предсказывала здесь «молча портила бы цепочку», то
         * есть обрыв. Замером **не подтвердилось**: уступка уходит в брошенный
         * круг, тот пуст, деструктор возвращается немедленно, и сосед живого
         * круга не успевает пробежать. Наблюдаемое - порядок, а не падение.
         *
         * Отмена одной задачи этого не различает вовсе: она отдаёт сегмент
         * через `adamas_segment_disown_base`, а тот снимает у кадра саму метку
         * `NURSERY`, и условие ниже её не видит. */
