...
fn suspends(expr: &Expr, known: &BTreeSet<FuncId>, quiet: bool) -> bool {
    let here = match expr {
        ...
        // всегда.
        //
        // Правило остаётся консервативным, но платит за него не отказ.
        // Чистая функция, которой оно досталось, заводит **свой** корень
        // трамплина - `alloca` на C-стеке, - и кадра в куче не кладёт
        // (`emit_c::applying`, `emit_llvm::applied`; трек A волны 3 Фазы 7).
        // Селекция по целям замыканий, названная там следующей ступенью
        // вопроса 74, не понадобилась: форму за указателем решает трамплин
        // замыкания, и решает статически.
        | Expr::Apply { .. } => true,
