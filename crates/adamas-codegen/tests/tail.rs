...
///
/// Он же - **мера** свидетеля остатка
/// ([`an_apply_in_tail_position_refuses_by_name_instead_of_signalling`]):
/// программа там та же самая плюс применение, и до этого трека она падала бы
/// и без него. То есть прежний свидетель остатка мерил сумму двух дефектов, а
/// не применение.
#[test]
...
///
/// **Снимают это два средства разом, и порознь ни одно не снимает ничего.**
/// Прежняя редакция этой шапки называла средством одно - трамплин на
/// применении, развилку (б) вопроса 192, - и это неверно: трек D волны 6
/// измерил все варианты правкой в порождённом C той же программы (умолчание
/// `adamas build`, стек 2 MiB, порог двоичным поиском; все варианты считают
/// одно число и оставляют ноль живых блоков).
///
/// | Вариант | Ячеек/виток | Проходит / роняет |
/// |---|---|---|
/// | как печатает эмиттер | 5 | 10 535 / 10 726 |
/// | прямой вызов известного кода, граница боксирует | 3 | 31 897 / 32 279 |
/// | плоский код лямбды, `adamas_apply` остаётся | 5 | 10 535 / 10 726 |
/// | биты прямо в слоте `adamas_apply` | 2 | 10 535 / 10 726 |
/// | прямой вызов **первой формой** плюс плоский код | 0 | 10⁸ проходят |
///
/// Читается так. **Снятие боксирования порога не двигает вовсе** - ни обёрткой
/// в трамплине, ни битами в слоте, - а ячейки двигает: третья и четвёртая
/// строки стоят на том же пороге, что первая. Порог двигает приставка, и берёт
/// она дугу только при дословно совпавшем прототипе (`Emitter::prototype`), то есть у
/// вызываемого обязаны быть плоские параметры, плоский ответ **и первая
/// форма**: у второй `prototype` печатает скрытые аргументы, а лямбда сегодня
/// второй формы всегда (`Lowerer::abstraction`). Что дело в приставке, а не в
/// снятых ячейках, показывает контроль - тот же вариант без неё даёт ноль
/// ячеек и 42 197 / 42 960 витков. Что прототипы обязаны совпасть **дословно**,
/// показывает clang 21: вариант, где плоский код оставлен второй формой,
/// отвергается сборкой - «cannot perform a tail call to function ... because
/// its signature is incompatible with the calling function».
///
/// Порог мерян (2 MiB, C-понижение): 10 681 виток проходит, 10 871 нет.
...
///
/// Вторая половина свидетеля - **как** он не берётся. До трека D волны 5
/// исчерпание стека выглядело сигналом 11 без единого слова о причине; теперь
/// рантайм называет её и уходит кодом 3 (`adamas-runtime/c/stack.c`). Лечением
/// это не является - программа по-прежнему не считает, - но отказ, который
/// называет себя, отличается от сигнала ровно тем, что по нему можно понять,
/// что случилось.
#[test]
fn an_apply_in_tail_position_refuses_by_name_instead_of_signalling() {
    let source = format!(
        "\
data Bool where
  True : Bool
