...
pub struct Binder {
    ...
    pub names: Vec<Name>,
    /// Остальные сомножители произведения кратностей: `(q * r z : a)`.
    ///
    /// Первый сомножитель лежит первым именем в `names`, как и одиночный
    /// параметр: форма у них общая, и различает их только `*`. Пусто у всех
    /// связываний, кроме тех, где произведение написано.
