...
#[test]
fn a_nested_record_type_does_not_cost_exponentially() {
    // `{x : …` читается и как группа связываний, и как тип записи, и различала
    // их попытка: тип поля разбирался целиком внутри группы, отказывал на
    // отсутствующей стрелке и разбирался заново записью. T(n) = 2·T(n−1), и
...
fn a_flat_list_that_unfolds_into_a_chain_is_bounded_too() {
    ...
    // Терм связывания не один: у параметра типового формера написано и
    // умолчание (§4.1). Пропущенное, оно не меряется ничем - предел молчал,
    // а глубина уезжала в переполнение стека.
    refused(
        "умолчание параметра",
        &format!(
            "type Big (a = {{ f0 : T{} }}) = T\n",
    ...
    // Хендлер есть применение элиминатора: спайн со звеном на каждую ветку,
    // а не одно звено на всю форму. Пока стояло одно, шестьдесят вложенных
    // хендлеров давали измеренную глубину 120 из 256 при настоящей около
    // пятнадцати тысяч, и проверка срывалась на них в переполнение стека.
    refused(
        "ветки хендлера",
        &format!(
            "run = handle prod with\n{}",
            repeat(300, |index| format!("  op{index} -> resume MkUnit\n"))
        ),
    );
    // Члены класса - тот же телескоп: класс элаборируется в тип записи, и поле
    // её стоит под предыдущими. Меры у арма не было вовсе, и семь тысяч членов
    // клали процесс по памяти - при том что семь тысяч операций эффекта на том
    // же входе отвечали отказом.
    refused(
        "члены класса",
        &format!(
            "class Big a where\n{}",
            repeat(300, |index| format!("  m{index} : a\n"))
        ),
    );
    // `module type` элаборируется тем же телескопом, что и класс.
    refused(
        "члены сигнатуры модуля",
        &format!(
            "module type Big where\n{}",
            repeat(300, |index| format!("  m{index} : T\n"))
        ),
    );
...
#[test]
fn a_case_without_branches_needs_no_block() {
    // Симметрично семейству без конструкторов: layout пустых блоков не делает,
    // а ядру разбор с нулём ветвей нужен - он и есть доказательство
    // необитаемости, и `absurd` пишется только им (§9 Фаза 1).
    //
    // Проверяются обе позиции: в конце файла блока не бывает по исчерпании
    // токенов, а в середине - по отступу следующего объявления. Ветки эти в
    // layout разные, и вторая ловится только так.
    assert_eq!(
        tree("absurd v = case v of\n").expect("разбор удался"),
        "(def absurd\n  (clause (v) (case v)))\n"
    );
    assert!(
        tree("absurd v = case v of\n\nafter = Zero\n").is_ok(),
        "разбор без ветвей не мешает следующему объявлению"
    );
}
