...
//! |---|---|---|
//! | входной файл | 2 | **15,9 мс** |
//! | `Std/Base` (его подключают все) | 2 | 15,7 мс |
//! | `Std/Base` | 10 | 40,1 мс |
//!
//! Цена идёт с **кода**, а не с файлов: те же 343 строки в двух файлах
//! перепроверяются за 17,8 мс, а в десяти - за 18,4 (0,07 мс на файл). Сто
//! миллисекунд бюджета интерактивности набираются к ~2500 строкам проекта.
//! Граф зависимостей и кэш готовых сигнатур за такие деньги не берут.
//!
//! Множитель у правки библиотечного модуля есть и назван: перепроверяется
//! каждый открытый буфер, который его подключил. При десяти окнах он даёт 2,5
