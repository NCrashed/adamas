...
//! - *Эффектная программа отвергается на элиминаторе хендлера, а не на форме*
//!   ([`a_handler_is_refused_by_its_own_name`]).
...
///
/// Хендлера здесь нет намеренно: он отвергается (см.
/// [`a_handler_is_refused_by_its_own_name`]), а посмотреть надо на форму. Метка
/// объявлена и не гасится нигде, потому что и не производится: row - **верхняя
/// граница** производимого, и `quiet u = Zero` под `{Ask}` законно. Функцию с
...
fn a_pure_function_carries_none() {
    ...
    // Применение значения из первой формы: брать скрытые ей неоткуда.
    let relayed = body(&text, "relayed");
    assert!(
        applied(&relayed).contains(", NULL, NULL, "),
        "первая форма передала скрытые аргументы, которых у неё нет:\n{relayed}"
    );
...
fn a_handler_is_refused_by_its_own_name() {
    ...
    assert!(
        !text.contains("форм"),
        "отказ всё ещё про форму функции, а она выбрана: {text}"
    );
}
