...
fn a_pure_function_roots_its_own_stack() {
    ...
    // Двенадцать, и каждый назван: две ячейки `Cons`, два `Succ` (ветка
    // отвечает дважды), замыкание `body`, кадр хендлера, два вектора evidence
    // - пустой корень чистой `runAsk` и расширенный им, - и **четыре кадра
    // продолжения**: три точки приостановки в теле (две операции и вложенный
    // `Cons`, чьё значение приходит позже) плюс одна у применения `k`.
    //
    // Последние четыре и есть цена дробления (решение 3 волны 4), названная в
    // числе: кадр - блок кучи, и ставится он на каждую точку приостановки, чьё
    // продолжение непусто. Снимет её §10 вопрос 74 - инлайнинг хвостово
    // резумптивного хендлера убирает и точку, и кадр.
