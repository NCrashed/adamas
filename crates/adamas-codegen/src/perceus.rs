...
//! без RC, но кратность `1` в этой системе аффинна и лежит на **связывании**:
//! `f : (1 x : T) -> T`, позванная от ω-значения, законна, и объект внутри `f`
//! разделён. Статическая уникальность здесь поэтому не выводится из кратности
//! одного связывания - см. отчёт трека.
