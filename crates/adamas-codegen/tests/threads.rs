...
-- Ответ операции круга обязан быть указательным, и причина у него своя. У
-- обычной операции плоский ответ с трека B волны 4 разворачивается из обёртки,
-- которую кладёт `resume` (§10 вопрос 191); за ответом **файбера** не стоит ни
-- одного `resume` - его кладёт завершение задачи, - и разбирать там нечего.
-- Число поэтому ездит через задачу в обёртке `Sum`; арифметика внутри остаётся
-- плоской.
effect Async where
  suspend : Unit
