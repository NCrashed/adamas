...
pub enum CommentKind {
    ...
    Block,
    /// `-- | документация` - открывает документирующий блок (§7.1).
    ///
    /// Различается **написанием**, а не положением: комментарий над
    /// объявлением документацией сам по себе не является. Счёт по корпусу
    /// (лог 2026-09-23): комментариев над объявлениями 1056 при 5240
    /// объявлениях, и документация среди них - примерно один блок из семи;
    /// остальные шесть суть ожидаемый ответ прогона, история правки, ссылка в
    /// §10 и рубрика. Правило «комментарий над объявлением и есть
    /// документация» выдало бы около 909 записей мусора.
    ///
    /// Помечается **первая** строка блока. Продолжают его обычные `--`, и
    /// собирает их [`crate::docs`]: разновидность отвечает на вопрос «где блок
    /// начинается», а не «какая строка в него входит».
    Doc,
}
