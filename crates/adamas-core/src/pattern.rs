...
impl Compiler<'_> {
    ...
    fn split(
        ...
        // Неконкретная кратность проходит **как есть**: дерево собирается
        // элаборацией, до всякой подстановки, и параметр `q` здесь ещё
        // переменная. Прочитанный единицей, он ею и оставался - подстановка
        // правит `Mult::Var`, а не записанную вместо него `One`, - и `foldr`
        // под `{q : Mult}` проверялся только при `q = 1`: ветвь связывала
        // хвост линейно, а рекурсивный вызов расходовал его `q` раз (§10
        // вопрос 41).
