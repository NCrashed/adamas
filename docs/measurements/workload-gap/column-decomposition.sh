...
#                     `ADAMAS_FLAG_SHARED` с атомарной загрузкой **в теле**, как
#                     её делал `adamas_is_unique` от трека I до вопроса 175;
...
#   6. та же ветвь, атомарная половина **вынесена** за `noinline` - так
#                     `adamas_is_unique` устроен после вопроса 175: **так виток
#                     выглядит сегодня**.
#
...
# оказалась того же порядка, что выигрыш первого. Точка 6 отвечает, из чего эта
# цена состояла: не из перехода, а из атомарной операции, оставленной там, куда
# смотрит инлайнер.
#
...
/* Разделяемая половина точки 6: та же операция, что в точке 4, но за вызовом.
 *
 * Модель здесь одна единица трансляции, поэтому «вынесена» пишется явным
 * `noinline`; в рантайме это `ADAMAS_SHARED_HALF` (`object.c`). */
...
floor "+ ветвь по флагу, атомарная половина в теле (трек I, до вопроса 175)" \
      "$DIR/mode4" "$CELLS" "$PASSES" 4
floor "+ счётчик при чтении (понижение до вопроса 171)" \
      "$DIR/mode5" "$CELLS" "$PASSES" 4
floor "ветвь по флагу, половина вынесена (понижение сегодня)" \
      "$DIR/mode6" "$CELLS" "$PASSES" 4
