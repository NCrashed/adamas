    ...
    loop {
        match cursor.peek() {
            ...
            Some('-') if cursor.dashes_start_a_comment() => {
                let start = cursor.mark();
                // `-- |` открывает документирующий блок (§7.1). Форму лексер
                // принимал и раньше - она законный комментарий, - поэтому
                // заводится здесь не форма, а её смысл, и корпус от этого не
                // двигается: вхождений было ноль.
                let opens_documentation = cursor.starts_with("-- |");
                while cursor.peek().is_some_and(|ch| ch != '\n') {
                    cursor.bump();
                }
                lexed.comments.push(Comment {
                    kind: if opens_documentation {
                        CommentKind::Doc
                    } else {
                        CommentKind::Line
                    },
...
mod tests {
    ...
    /// Маркер документации - ровно `-- |`, и соседние написания им не являются.
    ///
    /// Три написания, и все три несущие. `-- |` - решение владельца (§7.1, лог
    /// 2026-09-23). `---` есть линейка-рубрика, и стань она маркером, каждая
    /// рубрика корпуса объявила бы себя документацией следующего объявления.
    /// `--|` без пробела - **не** комментарий вовсе: кусок не целиком из
    /// минусов, и отобрать это написание у операторов нельзя, не трогая их
    /// лексику; отсюда и пробел в маркере.
    #[test]
    fn the_doc_marker_is_exactly_one_spelling() {
        use crate::token::CommentKind;
        let piped = lex("-- | Складывает.\nf = 1").expect("лексер справился");
        assert_eq!(piped.comments[0].kind, CommentKind::Doc);
        let rule = lex("----------\nf = 1").expect("лексер справился");
        assert_eq!(rule.comments[0].kind, CommentKind::Line);
        let glued = lex("--| Складывает.\nf = 1").expect("лексер справился");
        assert!(glued.comments.is_empty(), "`--|` прочтён комментарием");
        assert_eq!(glued.tokens[0].kind, TokenKind::Operator);
    }
