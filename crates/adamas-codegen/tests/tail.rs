...
///
/// Читается так. **Снятие боксирования порога не двигает** - ни обёрткой в
/// трамплине, ни битами в слоте, - а ячейки двигает: третья и четвёртая строки
/// стоят на том же пороге, что первая. Числа сняты gcc 15.3; clang 21.1.8 даёт
/// у первой, второй и третьей строк те же, а у четвёртой 15 875 / 16 257 -
/// полтора раза, то есть по-прежнему не гарантию.
///
/// Порог двигает приставка, и берёт она дугу только при дословно совпавшем
/// прототипе (`Emitter::prototype`), то есть у вызываемого обязаны быть плоские
/// параметры, плоский ответ **и первая форма**: у второй `prototype` печатает
/// скрытые аргументы, а лямбда сегодня второй формы всегда
/// (`Lowerer::abstraction`). Кольцо у программы трёхдужное, и приставку обязана
/// взять **каждая** дуга: тот же вариант с приставкой на одной из двух новых
/// дуг даёт 42 197 / 42 960 - ровно столько же, сколько без приставок вовсе.
/// Что прототипы обязаны совпасть **дословно**, показывает clang: вариант, где
/// плоский код оставлен второй формой, отвергается сборкой - «cannot perform a
/// tail call to function ... because its signature is incompatible with the
/// calling function».
///
