...
fn a_numeral_is_measured_by_its_value() {
    ...
    // обязан считать её значением - иначе короткая запись роняет процесс
    // глубиной терма, которого никто не видел.
    //
    // Мерит его элаборация, а не разбор, и переехало правило вместе с
    // примитивами (§4.11): величина зависит от **типа**, а типа разбор не
    // знает. Мутант «мерить по-прежнему в разборе» ловится соседом ниже:
    // тот же литерал под `Int64` обязан пройти.
    let text = format!(
        "{BASE}
big : Nat
big = 100000
"
    ...
    assert!(
        matches!(error, ElabError::UnaryLiteral { .. }),
        "получено {error:?}"
    );
}

#[test]
fn a_primitive_type_lifts_the_unary_limit() {
    // Ближайший проходящий сосед к отказу выше: то же число, тип другой.
    // Примитив (§4.11) представление имеет, разворачивать его унарно незачем,
    // и предела ему нет.
    let signature = program(&format!(
        "{BASE}
big : Int64
...
"
    ...
    // Параметр кратности стоит в **домене** связывания, а не в кодомене, и у
    // второго порядка это обычное место (§10 вопрос 41). Обход искал его лишь
    // в кодомене, поэтому всякий такой комбинатор получал предупреждение о
    // неиспользованном имени - при том что имя используется.
    assert!(
        warned(&format!(
            "{BASE}
applyTo : {{q : Mult}} -> (ω f : (q x : a) -> b) -> (q y : a) -> b
applyTo f x = f x

inner : {{q : Mult}} -> (ω f : (q x : a) -> b) -> Nat
inner f = Zero
"
        ))
        .is_empty(),
        "параметр кратности в домене предупреждения не даёт"
    );
}
...
#[test]
fn a_lambda_in_head_position_binds_instead_of_escaping() {
    // §10 вопросы 100 и 101. `(\b -> drop h) True` отвергалось как побег, хотя
    // ничего не возвращается: `Term::Lam` домена не хранит, тип головы не
    // выводился, а умолчание считало невыведенный результат функцией. Правило
    // §3.3 сообщало о чужом случае, потому что докладывалось первым.
    //
    // Теперь бета-редекс становится связыванием, и обе формы **пишутся**: та,
    // что была примером вопроса 100, - в том числе. Владение к ней и правда
    // отношения не имело.
    program(&format!("{BASE}\nplain : Bool\nplain = (\\b -> b) True\n"));
    program(&format!(
        "{BASE}
...
"
    ...
    // Граница: владеемый **аргумент**. Связывание редекса берёт тип синтезом, а
    // вставку `drop` решает написанный, поэтому здесь отказ - и он называет
    // свою причину, а не «нужна аннотация», которую негде написать.
...
fn a_handler_names_which_of_its_refusals_fired() {
    // §10 вопрос 104: причин отказа под хендлером было три на одно сообщение, и
    // лямбде оно называло **не ту** - «не производит `L`», хотя она производит,
    // а не выводился её тип (§10 вопрос 101). Четвёртое место - нить
    // параметризованного, где прежний текст говорил про написанное неправду:
    // вычислением нить быть не обязана вовсе.
    //
    // Лямбда с тех пор пишется: синтеза ей нет по-прежнему, но **ожидаемый**
    // тип у неё здесь есть, и проверка против него узнаёт ρ. Осталось две
    // причины, и каждая называет себя.
    let base = format!(
        "{BASE}{AMB}
effect Other where
  ping : Nat
...
"
    ...
    // Та же лямбда **без имени** - тоже пишется: ожидаемый тип у неё есть, и
    // называть вычисление больше не обязательно (§10 вопрос 104, вторая
    // половина). Линейное связывание при этом остаётся под правилом кратности:
    // одношотный хендлер проходит вычисление однажды.
    program(&format!(
        "{base}
tried : (1 n : Nat) -> Nat
tried n = handle (\\u -> pick n toss) with
  return v -> v
  toss -> resume True
"
    ));
...
";
    let signature = program(text);
    // Параметры метки у операции первыми, свои - следом. Полиморфна операция по
    // одному уровню, и это уровень **метки**: свой параметр живёт в нулевом
    // универсуме (§10 вопрос 83), потому что связать его в точке `handle`
    // нечем - метка о нём не несёт ничего, тип вычисления его не называет.
...
fn a_polymorphic_operation_is_handled_at_a_single_level() {
    ...
    // из §4.4.
    //
    // Берётся нуль, и берётся он **на объявлении операции**, а не только у
    // ветки (§10 вопрос 83). Порознь у них выходило по своей копии уровня, обе
    // стёртые, и расхождения никто не видел: вызов при `a := Type` проходил,
    // тогда как ветка ждала нулевой универсум.
    let signature = program(
        "\
data Bool where
  True : Bool
...
",
    ...
    // Уровней у элиминатора два: параметров метки нет вовсе, и оба несут
    // результат вычисления и ответ хендлера. Операция своего не приносит -
    // он заземлён на её объявлении.
    let handler = signature
        .lookup("#handle.Amb")
        .expect("элиминатор объявлен");
    assert_eq!(handler.level_arity, 2);
