...
//!
//! Что называется по имени, а не молчит: общая ветка (трек D), абортивная
//! (трек C), мультишот (трек E), параметризованный хендлер (трек D) и маска.
...
/// Абортивная ветка отвергается по имени, со ссылкой на трек C.
#[test]
fn an_abortive_branch_is_refused_by_name() {
    let source = format!(
        "{SHAPE}\
asked : {{Ask}} Nat
asked = ask
...
"
    ...
    let error = harness::compiled(&source).expect_err("абортивная ветка - трек C");
    let text = error.to_string();
    assert!(
        text.contains("не зовёт резумпцию") && text.contains("трек C"),
        "отказ не назвал ни вердикт, ни трек: {text}"
    );
}
