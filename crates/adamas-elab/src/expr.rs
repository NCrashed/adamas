...
impl<'a> Elaborator<'a> {
    ...
    pub(crate) fn module_members(
        ...
        let ty = if let Some(written) = first.ty {
            // Через `typing`, а не через общий путь объявления: подъём здесь
            // связать нечем. Свободное имя стало бы implicit-параметром члена,
            // а хвост row - параметром **самого** `module type`, чей тип его не
            // называет: обобщение идёт по типу определения, а члены живут в
            // теле. Отсюда закрытая row у члена сигнатуры и отказ на свободном
            // имени - оба названы долгом (лог 2026-09-03), и оба про одно.
