...
//!   ([`Shape::TailResumptive`]), но не против Rust: сосед на Rust есть только
//!   у чистой формы, а под хендлером сравниваются между собой две формы
//!   понижения. Цифра 1.5–2x требует эффектного соседа, которого писать не на
//!   чем.
...
//!
//! Формы три, и третья заведена не для симметрии. Мерить `handled` против
//! `pure` было бы враньём: между ними **два** различия, а не одно.
//!
//! Различие первое — операция. Различие второе — представление множителя.
//! Ветка хендлера идёт кадром, слот кадра единообразен (§4.11), и плоский
//! `Int64` через него не проходит: понижение отвергает такую программу словами
//! «ожидалось указательное значение, а пришло плоское». Боксирование значения
//! на границе замыкания — §5.1, §10 вопрос 158, и этим срезом оно не берётся.
//! Первая редакция стенда упиралась в это и точку `native/handled` не мерила
//! вовсе.
//!
...
//! обходной путь под хендлер лежит рядом, а не поверх.
//!
...
/// Форма программы: чистый сосед, он же с указательным множителем, и тот же
/// алгоритм под хендлером.
///
/// Все три считают одно число. Различие ровно одно на каждом шаге: `Pure` →
/// `Boxed` меняет представление множителя, `Boxed` → `TailResumptive` меняет
/// литерал на операцию `ask`, чья ветка зовёт `resume` в хвосте. Порознь,
/// потому что вместе они мерили бы сумму.
...
/// бы не операцию.
///
/// Арифметика уехала **внутрь** `scale`, и это не вкусовщина: у `handled`
/// операция дробит тело `scaled`, а куски тела ходят через трамплин
/// указателем. Оставь `mulInt64` после операции — и продолжение обязано
/// вернуть плоский `Int64` в позиции `adamas_value` (§4.11, §10 вопрос 158).
/// Дефект названного места записан в отчёте трека: понижение такую программу
/// не отвергает, а печатает C, который не собирается.
...
fn symbolic(criterion: &mut Criterion) {
    ...
    for shape in [Shape::Pure, Shape::Boxed, Shape::TailResumptive] {
        let name = shape.name();
        for depth in DEPTHS {
...
/// Три формы обязаны считать одно число.
///
...
fn same_answer() {
    ...
    for shape in [Shape::Boxed, Shape::TailResumptive] {
        assert_eq!(
            pure,
