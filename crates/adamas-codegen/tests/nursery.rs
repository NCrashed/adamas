//! Два инварианта питомника, переживших корпус (§5.2).
//!
...
//!
//! # Чем они мешали наблюдать себя
//!
//! Оба были при закрытии волны 5 Фазы 6 названы **выжившими мутантами**: сними
//! строку - корпус зелёный целиком, включая ноль живых блоков. Мешало каждому
//! своё, и лишь у первого - обрыв.
//!
//! *У первого различающая программа кончается обрывом*, а наблюдать обрыв ни
//! один прогон тогда не умел: и `harness::agreed`, и `harness::llvm_agreed`
//! роняют тест на ненулевом коде возврата, то есть обрыв у них поломка
//! окружения, а не наблюдение. Трек добавил `harness::c_printed` и
//! `harness::machine_printed` рядом с уже стоявшим `harness::llvm_printed`.
//!
//! *У второго обрыва нет вовсе*, и предсказание «молча портила бы цепочку»
//! замером не подтвердилось. Мешала ему форма программы: нужен второй,
//! **живой** круг, а отмена одной задачи этот инвариант не задевает по
//! построению (см. ниже).
//!
...
/// (снятый `ADAMAS_MARK_NURSERY` из условия подавления) уходит в брошенный
/// круг, тот пуст, и деструктор возвращается немедленно: след становится
/// `1 7 9 4` вместо `1 7 4 9`. Обрыва нет - есть переставленный порядок.
#[test]
