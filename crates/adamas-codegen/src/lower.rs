...
//! разбор, рекурсия, `let`. Хендлеров и резумпций здесь нет: кадра хендлера
//! понижение не ставит, и `#handle.L` отвергается по имени
//! ([`LowerError::Handler`]). Всё, что во фрагмент не входит, отвергается
//! названной причиной: молча посчитать не то хуже, чем не посчитать.
//!
...
//! двумя скрытыми аргументами. Это не то же, что «эффектов здесь нет»: функция
//! с непустой row понижается и эмитится, а отвергается только сам хендлер.
//!
