...
//!
//! Хендлер понижение ставит (трек B волны 4), а эту программу по-прежнему
//! отвергает, и причина названа: множитель у неё `Int64`, то есть **плоский**,
//! а ветка идёт кадром, чей слот единообразен (§4.11). Боксирование значения
//! на такой границе — §5.1, и этим срезом оно не берётся. Бенчмарк это
//! называет вслух, сверяя, что отказ именно про плоское. Появится
//! боксирование — точка `native/handled` начнёт мериться без правки файла.
    ...
    for shape in [Shape::Pure, Shape::TailResumptive] {
        ...
        for depth in DEPTHS {
            ...
            match program.lowered() {
                ...
                // Шов трека B: хендлер поставлен, а множитель у этой программы
                // плоский — `ask : Int64`, — и через ветку он не проходит.
                // Ветка идёт кадром, слот кадра единообразен (§4.11), и
                // боксирование этим срезом не берётся (§5.1). Отказ обязан
                // быть про **это**, а не про что-то ещё.
                Err(error) => {
                    let refusal = error.to_string();
                    assert!(
                        shape == Shape::TailResumptive && refusal.contains("плоское"),
                        "{name}/{depth}: понижение отвергло по неожиданной причине: {refusal}"
                    );
                    eprintln!(
                        "native/{name}/{depth}: не мерится — понижение отвергает: {refusal} (боксирование, §5.1)"
                    );
                }
            }
        }
