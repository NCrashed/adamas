...
pub enum ErrorKind {
    ...
    /// нечем: арифметики кратностей в сигнатуре нет (§10 вопрос 41). Тело
    /// тогда проверяется не при всех сочетаниях, а множество законных
    /// подстановок перестаёт быть произведением по параметрам - принять его
    /// значило бы обещать больше законных сочетаний, чем есть.
    #[error(
        "параметры кратности `{name}` связаны между собой: тело проверяется не при всех их сочетаниях, а выразить связь сигнатуре нечем"
    )]
    MultEntangled {
        /// Имя определения.
        name: Name,
