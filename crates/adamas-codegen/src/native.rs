...
/// С чем линковать сверх рантайма: секция `[link]` манифеста (§5.3, §7.1).
///
/// Зависимость-**пакет** и зависимость-**библиотека** - разные вещи, и потому
/// это отдельная секция, а не запись в `[dependencies]`. Пакет приносит
/// **модули**: у него есть префикс путей, git URL и коммит, и §7.3 говорит
/// ровно про него. Библиотека приносит **символы**: префикса у неё нет,
/// достать её нечем (она уже на машине или её ставит система сборки чужого
/// проекта), а версия её - soname, а не тег. Записанные в одну таблицу, они
/// заставили бы половину полей быть бессмысленными у каждой стороны.
///
/// Написание - то же, в котором их принимает компоновщик: `libraries = ["m"]`
/// есть `-lm`, `paths = ["vendor/lib"]` есть `-Lvendor/lib`. Выбрано оно ради
/// **машины**: `-lname` компоновщик разрешает в файл `libname.so`, и он же
/// открывается `dlopen`'ом. То есть `adamas eval` и `adamas build` требуют от
/// системы ровно одного и того же файла, а не двух разных написаний одного.
#[derive(Clone, Debug, Default)]
pub struct Linking {
    /// Каталоги поиска библиотек: `-L`.
    pub paths: Vec<PathBuf>,
    /// Библиотеки: `-l`, без `lib` и без расширения.
...
impl Native {
    ...
    /// Она же с чужими библиотеками из манифеста (§5.3).
    #[must_use]
    pub fn linking(self, linking: Linking) -> Self {
        Self { linking, ..self }
    }
    ...
    /// Дописывает чужие библиотеки: `-L` перед объектниками не нужен, а `-l`
    /// обязан стоять **после** них - компоновщик разрешает символы слева
    /// направо, и библиотека перед объектником не даёт ничего.
    ///
    /// Рядом с `-L` идёт `-rpath`, и это не то же самое: первый говорит, где
    /// искать **компоновщику**, второй - где искать **загрузчику**. Без второго
    /// программа собирается и не запускается («cannot open shared object
    /// file»), потому что каталог проекта загрузчику неизвестен - измерено
    /// прогоном, а не выведено. Названная цена: путь уезжает в бинарь
    /// абсолютным, и перенос бинаря на другую машину его не переживает. Для
    /// `adamas run` это верный размен - собранное запускают там же, где
    /// собрали, - а раскладка установки есть отдельная работа.
    fn linked_with(&self, command: &mut Command) {
        for path in &self.linking.paths {
            command.arg("-L").arg(path);
            let mut rpath = std::ffi::OsString::from("-Wl,-rpath,");
            rpath.push(path);
            command.arg(rpath);
        }
        for library in &self.linking.libraries {
