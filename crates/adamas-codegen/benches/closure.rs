...
///
/// Цикл двойной, и это вынуждено. Применение замыкания печатается с
/// `adamas_kont` на кадре, у которого берётся адрес, и сишный компилятор
/// хвостовой вызов из-за этого не сворачивает: цикл остаётся рекурсией.
/// Замерено порогом - 100 000 витков проходят, 200 000 роняют процесс
/// сигналом 11. Внутренний цикл поэтому короткий, а длину прогона набирает
/// внешний, глубину не копящий.
///
