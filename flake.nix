  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        # Настоящие чужие библиотеки милестоунов Фазы 8 (§9, «layered
        # wrapping»): корпус зовёт `zlib` (волна 2) и `libcurl` (волна 5).
        # Приезжают флейком по тому же правилу, каким приезжают LLVM и
        # редакторы, - инструмент, которым проверяется утверждение, не берётся
        # «из системы».
        #
        # Нужны они **дважды и по-разному**, и отсюда обе строки ниже.
        # Компоновщику довольно `-lz`: `NIX_LDFLAGS` уже несёт `-L` на них, и
        # понижения собираются без единой правки. А машина зовёт `dlopen`, и
        # `dlopen` каталогов `-L` не читает вовсе: без `LD_LIBRARY_PATH` она
        # отвечает «libz.so не загрузилась» там, где обе сборки прошли. То
        # есть договор трёх вычислителей на настоящей библиотеке держится
        # именно этой переменной.
        #
        # Список, а не один пакет, и выход у `curl` написан, а не подразумеван.
        # Умолчательный выход `pkgs.curl` есть **`bin`**, и `lib` в нём нет
        # вовсе: `libcurl.so` вместе с `libcurl.so.4` лежит в `out`. Машина
        # ищет ровно `libcurl.so` (`Linkage::candidates`), поэтому написано
        # `curl.out`; `curl.dev` рядом - заголовки и `pkgconfig`, которыми
        # живёт компоновщик.
        # SDL2 - милестоун Фазы 9 (волна 5): демо на нём. Строка **одна**, и
        # это не небрежность рядом с двумя у curl: умолчательный выход
        # `pkgs.SDL2` несёт и `lib/`, и неверсированный `libSDL2.so` (симлинк на
        # `libSDL2-2.0.so`), то есть ровно то, что ищет `dlopen` машины
        # (`Linkage::candidates`: `lib<имя>.so`). Заголовки не нужны вовсе -
        # порождённый C объявляет чужой символ сам, а не включает `SDL.h`.
        #
        # Экран для прогона не требуется: `SDL_VIDEODRIVER=dummy` открывает
        # окно без всякого сервера, и свидетель гейта ходит именно так. Это
        # выбор за воспроизводимость, а не обход: `xvfb-run` в shell'е есть, но
        # он поднимает целый X ради утверждения «окно открылось».
        foreignLibraries = [
          pkgs.zlib
          pkgs.curl.out
          pkgs.curl.dev
      ...
      in
      {
        ...
        devShells.default = pkgs.mkShell {
          ...
          env = {
            ...
            # `dlopen` машины (§5.3): каталогов `-L` он не читает, и без этой
            # переменной `libz.so` не открывается даже там, где `cc -lz`
            # собирает. Дистрибутив кладёт её в `/usr/lib`, nix - в store,
            # поэтому переменная нужна ровно dev-shell'у.
            LD_LIBRARY_PATH = pkgs.lib.makeLibraryPath foreignLibraries;
          };
