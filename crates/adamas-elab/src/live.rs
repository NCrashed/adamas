...
//! вопрос 71).
//!
//! Правило написано в расчёте на то, что **клауза и есть ветвь**: `f (Wrap h)
//! = …` и `f Other = …` - два разных тела, и каждое отвечает за себя само, а
//! на собранном дереве разбора то же самое потребовало бы живучести по ветвям.
//!
//! **Допущение снято `case`-выражением** (§10 вопрос 82, ревью 2026-09-02), и
//! это долг, а не свойство: вердикт здесь булев и на всё тело клаузы, поэтому
//! упоминание ресурса в одной ветви считается упоминанием во всех, и ветвь,
//! его не назвавшая, `drop` не получает. §3.3 требует закрытия на всех путях
//! выполнения; `case` завёл путь **внутри** того, что правило видит.
//!
...
impl<'a> Spent<'a> {
    ...
    fn in_expr<'e>(&self, name: &str, expr: &'e Expr, bound: &mut Vec<&'e str>) -> bool {
        match &expr.kind {
            ...
            ExprKind::Mask(inner) => self.in_expr(name, inner, bound),
