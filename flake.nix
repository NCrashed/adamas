  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        # Настоящая чужая библиотека милестоуна волны 2 Фазы 8 (§9, «layered
        # wrapping»): корпус зовёт `zlib`. Приезжает флейком по тому же
        # правилу, каким приезжают LLVM и редакторы, - инструмент, которым
        # проверяется утверждение, не берётся «из системы».
        #
        # Нужна она **дважды и по-разному**, и отсюда обе строки ниже.
        # Компоновщику довольно `-lz`: `NIX_LDFLAGS` уже несёт `-L` на неё, и
      ...
      in
      {
        ...
        devShells.default = pkgs.mkShell {
          ...
          env = {
            ...
            LD_LIBRARY_PATH = "${foreignLibrary}/lib";
          };
