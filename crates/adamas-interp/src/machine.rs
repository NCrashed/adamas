...
///
/// Символ, а не примитив языка: таблица есть часть рантайма FFI, и объявляется
/// она автором обычным `extern "C"` - тем же, каким объявляется всякий чужой
/// символ. Машина зовёт его **сама**, не ходя в `dlsym`: порождённой единицы у
/// неё нет, и таблица у неё своя (`crate::callback`). Понижения зовут
/// настоящую сишную функцию того же имени (`adamas-codegen/src/callback.c`).
pub(crate) const CALLBACK_REGISTER: &str = "adamas_callback_register";

/// Символ рантайма: освободить слот таблицы трамплинов. См.
/// [`CALLBACK_REGISTER`].
...
impl<'a> Machine<'a> {
    ...
    fn evaluating(env: &Env, term: &Rc<Term>, kont: &mut Kont) -> Step {
        match &**term {
            ...
            // Примитив сюда же: тип и литерал - готовые значения, а операция
            // застревает головой и считается тем же `eval::apply`, которым
            // машина применяет всякую нейтраль. Второго правила счёта заводить
            // нельзя - три вычислителя обязаны совпадать.
            Term::Var(_)
            | Term::Meta(_)
            | Term::Universe(_)
            | Term::Pi(..)
    ...
    fn outward(&self, name: &Name, spine: &[Elim]) -> Result<Option<Step>, RunError> {
        ...
        for (at, argument) in arguments.into_iter().enumerate() {
            ...
            let want = match &it.params[at] {
                ...
                Cross::Callback(form) => {
                    // Полная таблица у **занятия слота** (§5.3, уровень 3) -
                    // не отказ, а ноль: предел числа живых колбэков обязан быть
                    // виден в API обёртки, а не всплывать в рантайме. У всякой
                    // другой чужой функции ноль в позиции указателя на функцию
                    // был бы вызовом в никуда, и там остаётся отказ.
                    if &*it.symbol == CALLBACK_REGISTER && !crate::callback::vacant() {
                        bits.push(0);
                        continue;
                    }
                    let (guard, address, data) = self.registering(&it, &argument, form)?;
                    // Второй колбэк **со средой** в одном вызове - отказ, а не
                    // вторая среда: `userdata` в вызове один, и какой из двух
                    // колбэков он называет, не написано. Слово в слово то же,
                    // чем отвечает понижение (`lower::registering`). Два
                    // колбэка **без** среды при этом законны: у каждого свой
        ...
        // Таблица трамплинов - наш рантайм, а не чужая библиотека: к `dlsym`
        // эти два символа не идут вовсе (§5.3, уровень 3).
        if let Some(step) = self.tabled(&it, &bits, &mut registered)? {
            return Ok(Some(step));
        }
    ...
    /// Два символа рантайма, которыми живёт уровень 3 (§5.3).
    ///
    /// `None` - символ чужой, и зовётся он обычным путём. Иначе машина
    /// отвечает сама: таблица трамплинов у неё своя, и `dlsym` о ней не знает
    /// ничего. Понижения на этом же месте зовут настоящую сишную функцию того
    /// же имени (`adamas-codegen/src/callback.c`), и ответы обязаны совпасть.
    ///
    /// **Занять слот** значит оставить регистрацию живой после возврата:
    /// `CURLOPT_WRITEFUNCTION` ставится одним чужим вызовом, а зовётся другим,
    /// и ровно этим уровень 3 отличается от уровня 2. Ответ - адрес трамплина
    /// либо ноль, если слотов больше нет.
    ///
    /// **Освободить слот** значит снять регистрацию. Ответ - ноль при успехе и
    /// единица, если адрес таблице не принадлежит: снять незанятое есть ошибка
    /// обёртки, и разбирает её обёртка, а не рантайм.
    ///
    /// # Errors
    ///
    /// [`RunError`] - единицы в сигнатуре нет там, где она нужна ответу.
    fn tabled(
        &self,
        it: &Foreign,
        bits: &[u64],
        registered: &mut Vec<crate::callback::Registration>,
    ) -> Result<Option<Step>, RunError> {
        let word = match &*it.symbol {
            CALLBACK_REGISTER => {
                // Слот остаётся занятым: дроп регистрации его освободил бы на
                // возврате, то есть ровно тем скоупом, за который уровень 3 и
                // выходит.
                for guard in registered.drain(..) {
                    guard.kept();
                }
                bits.first().copied().unwrap_or(0)
            }
            CALLBACK_RELEASE => u64::from(!crate::callback::release(
                bits.first().copied().unwrap_or(0),
            )),
            _ => return Ok(None),
        };
        let Some(ty) = it.result else {
            return Ok(Some(Step::Return(self.unit()?)));
        };
        Ok(Some(Step::Return(Rc::new(Value::Prim(Prim::literal(
            ty, word,
...
/// число. Скалярное чтение это переживало - `arrayIndex` отдавал имя, и
/// разворачивало его следующее применение, - а **векторная** загрузка нет:
/// `simdLoad` собирает канон сразу из `n` ячеек, и подорожечная свёртка
...
///
/// Стёртых имплиситов у `simdSplat` два - дорожка и словарь, - у прочих три:
/// ширина, дорожка, словарь; у `simdLoad` и `simdStore` это длина колонки,
/// дорожка и словарь, а написанная ширина стоит четвёртой и разворачивается
/// заодно (она уже литерал). Всё, что после них, свёртке нужно развёрнутым.
