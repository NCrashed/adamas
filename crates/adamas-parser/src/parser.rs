...
///
/// **Откуда число.** Замер в **наименьшем** окружении, в котором язык работает:
/// порождённый поток с умолчательным стеком в два мегабайта (`cargo test`
/// гоняет крейты именно в нём) и **отладочная** сборка. Явно стек не задан
/// нигде, поэтому меньшего окружения у языка нет, а большее - главный поток
/// `adamas` и `adamas-lsp` - берётся от `ulimit -s`, то есть тоже не наше.
///
/// Там срывается **понижение**, и раньше всех прочих: спайн применений в 125
/// звеньев проходит, в 126 роняет процесс переполнением стека. Проверка типов
/// держит 208, машина - больше 208, разбор голых скобок - 380 входов; порог
/// ставит самый мелкий, и это `adamas-codegen`. Число то же самое воспроизводит
/// поток теста: `tests/depth.rs` показывает 125 против 126 в `cargo test` без
/// всякого `stack_size`.
///
/// Предел взят **64** - половина измеренного, - и половина эта не круглая, а
/// вынужденная: глубина написанного и глубина, в которую разворачивается
/// литерал, **складываются** (§10 вопрос 187). Половина оставлена литералу, чей
/// предел стоит в элаборации (`adamas-elab`, `expr::STRING_LIMIT`) и выбран под
/// **эти** 64: под ними проходит строка в 46 байт и роняет 47, а предел строки
/// стоит на 40.
///
/// С унарным литералом сумма **не** сходится, и это названо числом там же
/// (`expr::UNARY_LIMIT`): корпусу нужен литерал 100, а под 64 звеньями рвётся
/// уже 66. Опустить 64 до 26 значило бы отвергнуть корпус, который сегодня
/// требует 30.
///
/// **Прежнее число (256) было больше того, что язык выдерживает**, и вдвое: оно
/// опиралось на замер «1000-1300 звеньев в debug `check`», снятый в главном
/// потоке и до того, как появилось понижение. Величина, зависящая от профиля
/// сборки и от стека звавшего, поверхностью языка быть не может - отсюда выбор
/// наименьшего окружения.
///
/// Цена названа: ни спайна, ни блока, ни группы связываний, ни списка длиннее
/// 64 не напишет и сгенерированный код. Корпусу сегодня хватает **30** звеньев
/// и **26** входов спуска (обе меры сняты обходом `tests/golden`), то есть запас
/// двукратный. Если упрётся, снимать предел не нужно - достаточно сделать
/// потребителей итеративными, а их сегодня большинство рекурсивные (`infer_app`
/// и `eval` в ядре, понижение, наконец `Drop` у самой цепочки `Rc`).
...
impl<'a> Parser<'a> {
    ...
    ///
    /// Метка не пишется: её называют ветки. Написанная за `@` (§4.1) вхождение
    /// не выбирает - снимается всё то же первое, то есть внутреннее, - а
    /// закрепляет его: говорит, чем обязаны оказаться его аргументы.
