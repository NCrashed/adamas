    ...
    for decl in decls {
        ...
        match &decl.kind {
            ...
            DeclKind::Export(exported) => {
                ...
                // Отказ здесь громкий нарочно - проход [`exports`] идёт по
                // объявлениям **файла**, и молча пропущенный член модуля
                // означал бы экспорт, которого нет в объектнике.
                only_at_top(
                    within,
...
fn crossing(ty: &Term, signature: &Signature, span: Span) -> Result<Shape, ElabError> {
    ...
    while let Term::Pi(binder, _, written, row, codomain) = rest {
        ...
        if binder.mult == Mult::Zero {
            ...
            // объявлением на аргумент короче написанного: автор написал буфер, а
