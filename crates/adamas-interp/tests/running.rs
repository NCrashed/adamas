...
/// Чужой вызов машина отвергает **названной причиной**, а не молча (§5.3).
///
/// Отказ этот - граница **машины**, а не языка: проверка типов программу
/// приняла, оба понижения её берут и считают
/// (`adamas-codegen/tests/foreign_call.rs`), а позвать чужую функцию машине
/// нечем. До этой строки насыщенный чужой символ оставался нейтралью и
/// печатался **именем со спайном** - то есть выглядел как посчитанный ответ, -
/// и жанр этот в дереве уже назван соседним свидетелем: постулат над глубоким
/// значением печатается так же и печататься так обязан. Различает их ровно
/// `extern`.
///
/// Механизм похода наружу ставит свой трек (§10 вопрос 182); здесь только
/// граница, и текст её говорит про машину.
#[test]
fn the_machine_refuses_to_go_outside() {
    let source = format!(
        "{BASE}
effect Foreign
...
"
    ...
    assert!(
        matches!(&error, adamas_interp::RunError::Foreign { name } if name == "adamas_probe_mix"),
        "отказ обязан называть символ: {error}"
    );
    assert!(
        error.to_string().contains("машина наружу не ходит"),
        "текст отказа обязан говорить про машину, а не про язык: {error}"
    );
}
