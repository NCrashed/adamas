...
pub struct Crossing {
    ...
    pub symbol: Name,
    /// Связывания по порядку: `Some(ty)` - слово, `None` - единица.
    ///
    /// Длина - арность **адамасова** имени, а не сишного: единица связывание
    /// занимает, а аргумента чужой стороне не даёт (в домене она есть `(void)`).
    pub params: Vec<Option<crate::prim::PrimTy>>,
...
impl Crossing {
    ...
    pub fn carried(&self) -> Vec<crate::prim::PrimTy> {
        self.params.iter().copied().flatten().collect()
    }
}
