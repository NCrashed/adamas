...
static adamas_evidence *closing_evidence(adamas_frame *frame, adamas_frame *rest) {
    ...
    for (adamas_frame *below = rest; below != NULL; below = below->below) {
         ...
         *
         * **Свидетеля у половины про питомник нет, и это измерено мутантом:**
         * сними `NURSERY` из условия - корпус зелёный целиком. Различающая
         * программа уступала бы **из деструктора** брошенной задачи, а таких в
         * корпусе нет ни одной; без подавления она резала бы стек по кадру,
         * которого на нём нет, то есть молча портила бы цепочку. */
