...
//! драйвера.
//!
//! Шов **занят**: [`Pipeline::whole_program`] прикладывает рантайм битовым
//! кодом перед `opt`, и без этого схлопывать треку C нечего - `adamas_dup` для
//! оптимизатора непрозрачен, пока рантайм приезжает готовым объектником.
//!
...
impl Pipeline {
    ...
    /// потому что смысл её ровно в том, чтобы `opt` увидел тела рантайма. Без
    /// неё `adamas_dup` для оптимизатора непрозрачен, и трек C схлопывать нечего
    /// (`docs/phase7-plan.md`, пункт 2).
    ///
