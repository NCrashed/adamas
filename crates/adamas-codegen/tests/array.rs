...
/// Плоский элемент неизвестного типа наружу не выходит.
///
/// Внутри кадра он живёт буфером: `rotate` читает ячейку и кладёт её в
/// соседнюю, и ширина обоих движений одна. Вынести его из функции значило бы
/// **боксировать** (§5.1) - завести ячейку кучи под байты, чьей формы вызывающий
/// не знает, - и боксирования этот срез не делает. Граница названа отказом, а
/// не молчанием: молча вернуть указатель внутрь массива значило бы отдать
/// ссылку на блок, который дроп массива уже освободил.
const ESCAPING: &str = "\
type Int = Int64
...
/// Элемент неизвестного типа в ответе функции отвергается названной причиной.
#[test]
fn an_unknown_element_does_not_leave_its_frame() {
    let error = harness::written_text(ESCAPING).expect_err("элемент наружу не выходит");
    let text = error.to_string();
    assert!(
        text.contains("плоский элемент неизвестного типа"),
        "отказ не назвал причину представлением: {text}"
    );
}

/// Ближайший проходящий сосед: тот же элемент, не покидающий кадра.
///
/// Отличие от соседа сверху - одно: прочитанное кладётся обратно в массив, а
/// не отдаётся вызывающему. Понижение поэтому берёт ту же программу.
#[test]
fn the_same_element_staying_inside_passes() {
    assert_eq!(allocated_as_written("массив-внутри-кадра", GENERIC), 1);
}
