...
-- Запись обязана дожить до понижения самим значением: проекцию из известной
-- записи специализация сворачивает в поле (§10 вопрос 163), и `made.left`
-- границы больше не касается - ответ у свёртки тот же, что у машины.
main : Pair
main = made
";
