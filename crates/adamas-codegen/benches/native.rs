...
//!   привязкой размах между прогонами — 1.06.
...
//!   на глубине 16). Медиана пяти это переживает, среднее — нет.
...
//!   называет величину;
//! - разброс между прогонами шире разброса внутри выборки: померено
//!   окружение, а не программа.
//!
//! # Ответ на символьную строку §6, 2026-09-13
//!
//! **Нет: паритета с Rust нет.** Пять прогонов подряд, привязка к одному
//! P-ядру, медианы за вычетом своего пола (0.20 мс у `native`, 0.48 мс у
//! соседа):
//!
//! | глубина | сосед на Rust | понижение | отношение |
//! |---|---|---|---|
//! | 14 | 0.98 мс | 2.56 мс | 2.6 |
//! | 16 | 4.14 мс | 10.28 мс | 2.5 |
//! | 18 | **18.15 мс** | **43.32 мс** | **2.4** |
//!
//! Число повторяется: за пять прогонов `rust/18` лёг в 18.34–19.21 мс,
//! `native/pure/18` — в 42.24–44.63. У обоих размах между прогонами (4.6% и
//! 5.5%) того же порядка, что интервал внутри выборки (2.5% и 0.6–8.9%), то
//! есть меряется программа, а не запуск.
//!
//! §6 — цель, а не констатация, и эта её строка сегодня не выполнена: разрыв
//! **2.4 раза не в нашу пользу** и держится на всех трёх глубинах. Прежние
//! два чтения были получены другим протоколом и потому несравнимы с этим:
//! «в полтора раза быстрее» (2026-09-11) — соседом, померенным на взболтанной
//! куче, «втрое медленнее» (2026-09-12) — соседом на чистой, но без цены
//! запуска процесса, которую понижение платило.
//!
