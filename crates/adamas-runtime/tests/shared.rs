...
/// Новая область по переиспользованному адресу - **не** прежняя.
///
/// Дефект, которого не показал бы ни ответ, ни санитайзер, и найден он не
/// свидетелем, а раундовым стендом замера. Область есть блок `malloc`;
/// освобождённый блок той же ширины `malloc` выдаёт снова, и рука воркера,
/// сверяющая один адрес, принимает новую область за свою прежнюю. Дальше она
/// укладывает в кусок, **которого новая область никому не раздавала**: курсор
/// новой стоит на нуле, а рука пишет по старым смещениям. Два воркера получают
/// одни байты, и курсор при этом выглядит верным.
///
/// Форма, в которой это достижимо, §3.6 называет сама: переиспользование блока
/// **между** регионами (`withPooledRegion`, игровой цикл). Кадр за кадром -
/// новая область на месте прежней.
///
/// Наблюдаемое - `used` новой области: у честной руки первая укладка берёт
/// кусок, то есть 512 байт; у руки, принявшей новую за старую, курсор новой не
/// двигается вовсе.
#[test]
fn a_new_area_at_a_reused_address_is_not_the_old_one() {
    unsafe {
        adamas_stat_reset();
        // Раундов несколько, потому что переиспользование адреса зависит от
        // `malloc`: первая область такой ширины приходит `mmap`'ом и уходит
        // `munmap`'ом, а последующие - из кучи, и вот они адрес и повторяют.
        // Различающим прогон становится на первом же повторе; сколько их
        // вышло, печатается.
        ...
        for round in 0..8i64 {
            ...
            seen.push(area);
            let (area, at) = placed(area, round);
            assert_eq!(at, 0, "раунд {round}: укладка новой области идёт с нуля");
            assert_eq!(
                adamas_shared_used(area),
                512,
                "раунд {round}: новая область не раздала куска - рука приняла \
                 её за прежнюю по совпавшему адресу"
            );
            assert_eq!(fetched::<i64>(area, 0), round);
            adamas_drop(area, Some(releasing));
        }
        assert_eq!(adamas_stat_live(), 0);
        assert!(
            reused > 0,
            "адрес области не повторился ни разу за восемь раундов: прогон \
             верен, но различающим не был - без повтора номер области ничего \
             не сторожит"
        );
        eprintln!("адрес области повторился в {reused} раундах из 8");
    }
}
