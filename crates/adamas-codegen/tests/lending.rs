...
///
/// «Переживает» здесь значит **ровно вызов**: дроп стоит сразу за узлом, и
/// дольше заём не живёт ничем. Что бывает с API, читающим адрес позже,
/// показывает `a_pinned_buffer_agrees_with_the_machine` ниже.
#[test]
