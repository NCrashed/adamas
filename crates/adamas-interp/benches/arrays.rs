...
//! |---|---|---|
//! | `array_new_65536` | `arrayNew` на 65 536 ячеек и одно чтение | спайн заводил узел, блок считает 256 КиБ: **дороже** |
...
//! который проверяется.
//!
//! Что читается отношением `column_256_4 / column_64_4`. Спайн читал ячейку
//! **поиском** от вершины цепочки, то есть проход по колонке стоил квадрата;
//! блок читает по смещению, но запись копирует байты, то есть проход стоит
//! квадрата снова - и вопрос не в классе, а в постоянном множителе. Отвечает
//! на него прогон, а не рассуждение.
