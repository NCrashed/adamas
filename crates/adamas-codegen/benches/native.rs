...
//!
//! Разницу точек это не трогает: она 25 мс при разбросе 2.8.
//!
//! # Ответ на символьную строку §6, 2026-09-13
//!
//! **Нет: паритета с Rust нет.** Семь прогонов подряд, привязка к одному
//! P-ядру, медианы за вычетом своего пола (0.20 мс у `native`, 0.48 мс у
//! соседа):
//!
//! | глубина | сосед на Rust | понижение | отношение |
//! |---|---|---|---|
//! | 14 | 1.00 мс | 2.57 мс | 2.6 |
//! | 16 | 4.18 мс | 10.32 мс | 2.5 |
//! | 18 | **18.39 мс** | **43.32 мс** | **2.4** |
//!
//! Ответ устойчив к разбросу, а не только к медиане: худшая для понижения
//! пара из семи прогонов даёт 2.5, лучшая — 2.0. Паритета нет ни в одной, и
//! порядок разрыва держится на всех трёх глубинах.
//!
//! §6 — цель, а не констатация, и эта её строка сегодня не выполнена: разрыв
//! **2.4 раза не в нашу пользу**. Прежние два чтения получены другим
//! протоколом и потому с этим несравнимы: «в полтора раза быстрее»
//! (2026-09-11) — соседом на взболтанной куче, «втрое медленнее»
//! (2026-09-12) — соседом на чистой, но без цены запуска процесса, которую
//! понижение платило.
//!
