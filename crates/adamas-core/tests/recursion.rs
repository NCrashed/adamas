...
use adamas_core::term::{Binder, Branch, Case, Term};
...
/// Граница запрета: через `let` при `q = ω` нетотальное значение доходит до
/// выведенного типа.
///
/// Значение `let` проверяется при `q · σ`, поэтому `loop zero` при `ω` законно;
/// дальше оно уходит в окружение, и выведенный тип тела его подхватывает.
///
/// **Обещание §4.7 при этом держится, и держат его два правила** (§10 вопрос 43,
/// измерено 2026-09-05). Стёртый фрагмент защищён тем же `q · σ`: при `q = 0`
/// значение проверяется при нуле, и правило ссылки срабатывает. А определение,
/// донёсшее нетотальное до типа, само получает отрицательный вердикт и в тип уже
/// не попадает. Остаток - застрявший тип **внутри** нетотального определения:
/// разворачивать `loop` некому, наружу он не выходит.
#[test]
fn a_partial_value_still_reaches_a_type_through_let() {
    ...
    assert_eq!(
        ...
        "остаток заперт вердиктом и стёртым фрагментом - см. доккомментарий"
    );
}
