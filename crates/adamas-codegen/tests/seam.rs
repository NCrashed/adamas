...
///   вовсе - `adamas.h`, `SUPPRESSED`), а ветка - те, которых не связывает
///   (синтезированный триггер приостановленного вычисления). Владельца у них
///   в IR нет: массив этот - соглашение о вызове, а не узел.
/// - `{held}` - уборка обрыва в полёте (`Emitter::escape`). Узла у неё в IR нет
///   и быть не может: обрыв снимает **C-кадры**, которых в IR нет вовсе, - это
///   цена направленного стиля (`adamas.h`, «Обрыв в полёте»). Мнения о владении
///   эмиттер тут не заводит, он повторяет счёт `perceus`: связывание приходит с
///   одной ссылкой, `Dup` добавляет, вхождение и `Drop` отнимают. Уйдёт вместе
///   с дроблением тел, когда C-кадров между операцией и хендлером не станет.
///
