...
mod tests {
    ...
    /// Какие написания маркера документации лексер принял бы уже сегодня.
    ///
    /// Разновидности «док» в языке нет (§7.1, `adamas doc`; трек D волны 3
    /// Фазы 9 её мерил и решения не принимал). Что из этого следует для
    /// будущего выбора, видно только отсюда: два кандидата в маркер лексер
    /// **уже** читает комментарием, третий - нет, и ценой формы это решается
    /// по-разному.
    #[test]
    fn candidate_doc_markers_already_lex() {
        use crate::token::CommentKind;
        // `-- |`: кусок из минусов - `--`, вертикальная черта стоит за
        // пробелом и в него не входит. Комментарий, и правило не тронуто.
        let piped = lex("-- | Складывает.\nf = 1").expect("лексер справился");
        assert_eq!(piped.comments[0].kind, CommentKind::Line);
        // `---`: кусок целиком из минусов и длиннее двух. Тоже комментарий -
        // и это же написание есть линейка-рубрика.
        let rule = lex("----------\nf = 1").expect("лексер справился");
        assert_eq!(rule.comments[0].kind, CommentKind::Line);
        // `--|` без пробела - **не** комментарий: кусок `--|` не целиком из
        // минусов. Отобрать это написание у операторов нельзя, не трогая их
        // лексику.
