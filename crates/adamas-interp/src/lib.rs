...
pub enum RunError {
    ...
    /// вшитого имени, и требует от него ровно двух вещей: один конструктор и
    /// одно поле под номер файбера. Не сошлось - отказ на месте, а не значение,
    /// про которое неизвестно, чем оно окажется.
    #[error(
