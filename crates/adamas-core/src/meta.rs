...
pub struct Metas {
    ...
    /// Водораздел расширяемости хвостов row (§10 вопрос 170).
    ///
    /// Дырка **моложе** водораздела заведена текущей проверкой тела -
    /// инстанциация места вызова либо ожидание позиции, - и погашение вправе
    /// класть в неё метки расширением. Дырка старше принадлежит написанному
    /// типу объявления: расширить её значило бы молча вписать эффект в
    /// сигнатуру. Отметку ставит элаборация на входе в тело клаузы; до первой
    /// отметки (`None`) не расширяемо ничего.
    row_floor: Option<u32>,
...
impl Metas {
    ...
    /// Ставит водораздел расширяемости хвостов row (§10 вопрос 170).
    ///
    /// Зовётся на входе в тело клаузы: всё, что заведено раньше, - дырки
    /// написанного типа, и погашение их не расширяет.
    pub fn mark_row_floor(&mut self) {
        self.row_floor = Some(self.limit());
    }

    /// Вправе ли погашение класть метки в эту дырку расширением (§10
    /// вопрос 170): дырка моложе водораздела - инстанциация текущей проверки.
    #[must_use]
    pub fn row_extendable(&self, meta: RowMeta) -> bool {
        self.row_floor.is_some_and(|floor| meta.0 >= floor)
    }
    ...
    pub fn release(&mut self) {
        ...
        self.slots.clear();
        // Водораздел расширяемости не переживает границу: он про тело
        // **этого** объявления, а у следующего своя отметка.
        self.row_floor = None;
    }
