...
//! Поэтому в [`Definition`](crate::sig::Definition) поля нет, а проверка
//! зовётся снаружи. Отдаёт она при этом не «нет», а причину и маршрут:
//! диагностика §5.1 обязана называть место.
//!
//! Зовёт её элаборация - у **всякого** определения с телом, а не только у
//! написавшего `@fbip`: §7.2 просит показывать переиспользование ячейки там,
//! где атрибута нет, и ответ на оба вопроса обязан быть один. Считается он
//! один раз ([`report`]), кладётся в [`crate::sig::Reuse`], и атрибут читает
//! оттуда же, откуда читает подсказка: разойтись им негде по построению.
//!
...
/// Что обход нашёл в теле: где ячейка переписывается и где не смогла.
///
/// Обе половины нужны, и нужны **разным** читателям. Атрибут `@fbip` спрашивает
/// только вторую: несовместимость есть отказ. Подсказка §7.2 спрашивает первую -
/// «this Node allocation is reused» указывает на построение, занявшее слот, а не
/// на определение целиком.
///
/// Обход останавливается на первой несовместимости, как останавливался и до
/// того, как половины разделились: в [`Report::rewrites`] поэтому лежит то, что
/// встретилось **до** неё. Иначе у атрибута сменился бы названный отказ - он
/// называет первый.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct Report {
    /// Маршруты до мест, где построенная структура заняла слот разобранной.
    pub rewrites: Vec<Vec<Frame>>,
    /// Первая несовместимость, если она есть.
    pub blocked: Option<Incompatible>,
}

/// Что тело делает с разобранной ячейкой (§5.1).
///
/// `body` - терм определения, тот самый, что уходит в сигнатуру: маршруты
/// считаются по нему и ложатся на дерево разбора, собранное [`crate::pattern`].
#[must_use]
pub fn report(signature: &Signature, body: &Term) -> Report {
    let mut walk = Walk {
        signature,
        body,
        rewrites: Vec::new(),
    };
    let blocked = walk.region(body, 0, &mut Vec::new(), &mut Vec::new()).err();
    Report {
...
/// Совместимо ли тело с FBIP (§5.1) - половина [`report`], нужная атрибуту.
///
/// # Errors
///
...
pub fn compatible(signature: &Signature, body: &Term) -> Result<(), Incompatible> {
    report(signature, body).blocked.map_or(Ok(()), Err)
}
...
struct Walk<'a> {
    ...
    body: &'a Term,
    /// Где построенное заняло слот разобранного - в порядке обхода.
    rewrites: Vec<Vec<Frame>>,
}
...
impl Walk<'_> {
    ...
    fn region(
        ...
        if let Some((built, fields)) = self.built(term) {
            if fields > 0 && !slots.is_empty() {
                match take(slots, &built, fields) {
                    // Слот занят - ячейка переписана, и место это показывают
                    // читателю (§7.2). Внутрь `take` этого не убрать: маршрут
                    // знает обход, а не арифметика слотов.
                    Ok(()) => self.rewrites.push(route.clone()),
                    Err(fault) => {
    ...
    fn case(
        ...
        let body = self.body;
        let alive = scrutinee.is_some_and(|it| self.uses(body, 0, it) > 1);
