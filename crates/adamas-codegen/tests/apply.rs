...
//! - *Вторая форма не расползлась* ([`the_second_form_does_not_spread`]):
//!   число функций второй формы у программы одно и то же, считает его
//!   понижение, и правка эмиттера его не трогает. Мера у трека 169 была
//!   «экземпляров у имени не больше двух»; здесь она строже - **ноль
//!   изменившихся форм на весь корпус**.
...
/// Вторая форма не расползлась: форм эмиттер не трогает вовсе.
///
/// Считается это по корпусу, а не по одной программе, и считается **числом**:
/// формы живут на функциях, раздаёт их понижение (`lower::form` плюс спрос
/// места вызова, вопрос 169), и эмиттер их только читает. Мера - сумма функций
/// второй формы по всем программам корпуса, взятым LLVM-путём; рядом печатается
/// сумма применений, стоящих в **первой** форме, - тех самых мест, которые
/// маршрут «дать окружающей вторую форму» превратил бы в кадры.
#[allow(
    clippy::unwrap_used,
    reason = "заготовка теста: отказ здесь означает сломанный корпус"
)]
...
fn the_second_form_does_not_spread() {
    ...
    for path in &fixtures {
        ...
        if harness::llvm_text(&name, &source).is_err() {
            continue;
        }
    ...
    eprintln!(
        "корпус LLVM-пути: программ {programs}, функций второй формы {detached}, \
         применений в первой форме {pure_applications}"
    );
    // Применения в первой форме есть - иначе свидетель зелен по пустоте, и
    // «вторая форма не расползлась» не значило бы ничего.
    assert!(
        pure_applications > 0,
        "применений в первой форме нет: считать нечего"
    );
    // Число второй формы записано прогоном и сверяется прогоном же: сдвинется
    // оно - значит форму кто-то раздаёт иначе, и сказать об этом обязан тест, а
    // не следующий сквозной замер.
    assert_eq!(
        (programs, detached, pure_applications),
        (88, 739, 92),
        "мера форм сдвинулась: считано по корпусу выше"
    );
}
