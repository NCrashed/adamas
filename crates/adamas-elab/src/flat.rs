...
impl Why {
    /// Хвост сообщения. Форма согласована с тем, о чём он бывает: рекурсия,
    /// владение и абстрактность - всегда про **названный** тип, замыкание и
    /// переменная - всегда про безымянную форму.
    const fn face(self) -> &'static str {
        match self {
            ...
            Self::Abstract => "объявлен без представления: тела нет либо оно запечатано",
            Self::Rigid => "ещё ничем не определена - напишите `{Flat …}` в контексте",
            Self::Open => "оставлена открытой хвостом, и полей у неё не перечислить",
            Self::Alien => "значений не имеет, и укладывать в ней нечего",
            Self::Deep => "вложен глубже, чем вывод готов идти",
        }
    }
}
