...
//!
//! Что называется по имени, а не молчит: мультишот (трек E). Абортивная ветка
//! отсюда ушла - её берёт трек C, и свидетели её в `abortive.rs`; общая ветка и
//! параметризованный хендлер - трек D и `general.rs`; маска - `mask.rs`.
...
/// изнутри наружу. Сойдись порядок записей наоборот - ответил бы внешний.
///
...
fn a_pure_function_roots_its_own_stack() {
    ...
    // Девять, и каждый назван: две ячейки `Cons`, два `Succ` (ветка отвечает
    // дважды), замыкание `body`, кадр хендлера, два вектора evidence - пустой
    // корень чистой `runAsk` и расширенный им - и **один кадр продолжения** у
    // применения `k`: граница замыкания динамическая и точкой приостановки
    // остаётся.
    //
    // До первой ступени вопроса 74 кадров было четыре (блоков - двенадцать):
    // программа тиха, ветка хвостовая, и операция перестала быть точкой
    // приостановки - три кадра дробления (две операции и вложенный `Cons`,
    // чьё значение приходило позже) сняты, обе операции бегут на месте.
    assert_eq!(allocated, 9, "цена хендлера в блоках изменилась");
}
...
///
/// Резумпция **не** в хвосте намеренно: с хвостовой веткой программа тиха
/// (§3.4, вопрос 74), обрыва не существует по построению, и плоский ответ с
/// операцией законно компилируется - граница принадлежит нетихой программе.
#[test]
