...
///
/// Обещающих атрибутов у объявления нет ни одного - ни `nounwind`, ни
/// `readnone`: что чужая функция делает, компилятору неизвестно, и обещать за
/// неё нечего.
///
/// Стоит ровно один, и он **запрещающий**: [`NOBUILTIN`]. Без него оптимизатор
/// вправе узнать чужой символ по имени и посчитать его сам, а это ровно то
/// расхождение двух понижений, ради которого C-сторона собирается
/// `-fno-builtin` (`native.rs`, `PROGRAM_FLAGS`). Измерено: `opt -O2` без
/// атрибута сворачивает `pow(2.0, 10.0)` и `labs(-7)` на месте, а `cbrt(27.0)`
/// не сворачивает; с атрибутом не сворачивает ни одного из трёх. Ответы у
/// свёрнутых сходятся с библиотекой - LLVM считает их хостовой libm, - но
/// «сходятся сегодня» и «не расходятся по построению» это разные обещания, а
/// §5.3 обещает **вызов**.
...
/// вариадический прототип у C: `al` выставляет кодогенератор, и решает он это
/// по типу функции.
///
/// Разница с C одна и она в **месте вызова**: у `.ll` вариадический вызов
/// пишет тип функции целиком - `call i32 (i64, i64, i64, ...) @snprintf(…)`.
/// Требует этого **не синтаксис**, и это измерено мутантом: без типа функции
/// `llvm-as` принимает вызов молча - тип у названной глобали известен, - и
/// прогон доходит до верного ответа. Требует этого `al`: у вызова **без** типа
/// функции `llc -O2` не печатает `movb $1, %al` вовсе, у вызова с типом -
/// печатает, обеими цепочками (`opt`+`llc` и один `llc`). То есть
/// вариадичность вызова решает тип **при вызове**, а не при объявлении, и
/// «работает» здесь значит «`al` случайно оказался ненулевым».
///
/// У обычного вызова тип функции не печатается: он необязателен и ABI не
/// меняет.
...
///
/// Стоит у **объявления**, а не у каждого места вызова: `CallBase::isNoBuiltin`
/// спрашивает обе стороны, а объявление одно на символ - забыть его у одного из
/// мест вызова так нельзя по построению.
const NOBUILTIN: &str = "nobuiltin";
...
impl<'a> Builder<'a> {
    ...
    fn foreign(&mut self, function: ForeignId, arguments: &[Expr]) -> Result<String, LlvmError> {
        ...
        // Обязателен он не синтаксисом, а ABI: без него `llvm-as` принимает
        // вызов молча, а `llc` не выставляет `al` вовсе - см. [`variadic_list`],
        // там измерение. У обычного вызова тип функции ничего не меняет, и
        // печатать его значило бы переписать всякий чужой вызов ради формы.
