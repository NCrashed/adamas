...
",
    ...
    .to_string();
    // Прежде отказ звучал «вычисление под хендлером»: плоский ответ ветки
    // `return` не пересекал границу площадки. Обёртка примитива (§10 вопрос
    // 159) эту границу сняла, программа идёт глубже и упирается в следующую
    // названную - обрыв в функции с плоским ответом, тот же §4.11.
    assert!(
        flat_computation.contains("операция в функции с плоским ответом"),
