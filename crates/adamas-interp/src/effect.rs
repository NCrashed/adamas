...
//!
//! **Названная граница.** Хендлер ищется ближайший по стеку, а §3.4 при
//! одноимённых метках отдаёт вызываемому **внешнее** вхождение: длина `Λ` в
//! правиле погашения и есть смещение вектора evidence. Расходятся ответы только
//! у вложенных хендлеров одной метки; погашение `|Λ|` уже считает, но в терме
//! не сохраняет, и сохранить её - работа evidence-трансляции.
