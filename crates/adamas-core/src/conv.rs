...
fn same_head(
    ...
    match (left, right) {
        ...
        // Плоский блок (§4.11) равен блоку **побайтно**: конвертируемость есть
        // равенство значений, а два `arrayNew 3 zero` суть одно значение,
        // лежащее в двух блоках. Тождество блока наблюдаемо только чужой
        // стороной (§5.3), и спрашивать его здесь значило бы объявить
        // `arrayNew 3 zero` неконвертируемым с самим собой.
        (Head::Block(a), Head::Block(b)) => a.alike(b),
        // Операции региона - тем же правилом (§3.6).
        (Head::Region(op_a), Head::Region(op_b)) => op_a == op_b,
        // Операции над вектором - тоже (§4.9).
...
fn rigid(
    ...
    match (&**left, &**right) {
        ...
        // Примитив сравнивается сам с собой: у типа - именем, у литерала -
        // битами. Побитовое равенство плавающих здесь не приближение, а ровно
        // семантика `Eq Float` из §4.3: `0.0` и `-0.0` различны, `nan` равен
        // себе. Биты хранятся обрезанными по ширине типа, поэтому два
        // написания одного значения дают одно представление.
