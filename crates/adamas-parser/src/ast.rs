...
/// Чем соединены параметры в позиции кратности (§10 вопрос 41).
///
/// `Mixed` - написаны оба знака сразу. Разбору такое отличить нетрудно, а
/// смысла у него нет: приоритета между `*` и `+` в позиции кратности не
/// заведено, и выражение из двух ярусов там не хранится.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum Grade {
    /// `q * r` - произведение.
    Product,
...
pub struct Binder {
    ...
    ///
    /// Первое из них бывает не связыванием, а параметром кратности: `(q x : a)`
    /// - это `q` и `x`, и различает их не форма, а объявление (§10 вопрос 41).
    pub names: Vec<Name>,
    /// Остальные части выражения кратности: `(q * r z : a)`, `(q + r z : a)`.
    ///
    /// Первая часть лежит первым именем в `names`, как и одиночный параметр:
    /// форма у них общая, и различает их только знак. Пусто у всех связываний,
    /// кроме тех, где выражение написано.
    pub factors: Vec<Name>,
    /// Знак, соединяющий части: `None` - выражения нет вовсе.
...
pub enum ExprKind {
    ...
    Handle {
        ...
        /// одному эффекту. Снимается в обоих случаях первое вхождение;
        /// написанные аргументы его не выбирают, а закрепляют - говорят, чем
        /// оно обязано оказаться, и не совпало значит отказ.
