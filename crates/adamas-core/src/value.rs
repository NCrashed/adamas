...
/// байты: без него `arrayNew 4000000000 x` в типовой позиции заставил бы
/// `adamas check` выделить четыре гигабайта там, где прежде он выделял узел.
#[derive(Debug)]
pub struct Block {
    /// Тип ячейки.
    ty: PrimTy,
...
impl Block {
    ...
    /// (`compress2` над фикстурой корпуса - килобайты), а `adamas check`,
    /// наткнувшись на литеральную длину в типовой позиции, дороже мегабайта не
    /// заплатит.
