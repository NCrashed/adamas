...
//!
//! # Цена колбэка, четвёртая пара (2026-09-21, трек C волны 3, три прогона)
//!
//! Четвёртая пара мерит **колбэк уровня 1** (§5.3). Обе её половины ходят за
//! границу и зовут один и тот же чужой цикл `adamas_bench_through(turns,
//! step)`; вызов внутри него индиректный у обеих, и различаются они ровно тем,
//! кого он зовёт - нашу экспортированную `step` либо свою `adamas_bench_local`
//! с тем же телом. Разность есть цена того, что вызываемый **наш**.
//!
...
//!
//! §5.3 обещает здесь ноль, и **ноль подтвердился**: разность меняет знак между
//! прогонами, то есть лежит внутри разброса у обоих понижений. Держится он на
//! том, что обёртка экспорта есть один прямой вызов внутренней функции, и
//! встраивают её обе цепочки - `cc` по `-O2 -flto`, `opt` по `internal`.
//!
...
const LOADS: [(&str, &str); 9] = [
    ...
    ("callback", include_str!("crossing/callback.adamas")),
    (
        "callback-native",
...
/// Четвёртая мерит **колбэк** (§5.3, уровень 1), и обе её половины тоже ходят
/// за границу: чужой цикл один и тот же, вызов в нём индиректный у обеих, и
