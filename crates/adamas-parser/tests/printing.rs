...
fn a_long_application_spine_prints_without_recursion() {
    ...
    // поэтому у спайна предел свой - 256 (§10 вопрос 62). Печать по спайну
    // идёт циклом и на предельной длине рекурсией не пользуется; проверяется
    // ровно граница, потому что за ней текста для печати уже не бывает.
    let text = format!("f = g{}\n", " x".repeat(256));
