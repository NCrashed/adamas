...
///
/// Разошлись они только типом нагрузки. `Flat Bit` типовая сторона **выводит**:
/// перечисление из двух конструкторов занимает байт (§4.11). Плотной укладки у
/// семейства в понижении при этом нет - `Repr` несёт примитив либо запись из
/// примитивов, и тега среди них не бывает (§10 вопрос 157).
const TAGGED: &str = "\
data Bit where
  Off : Bit
  On : Bit
...
/// Нагрузка, чью укладку понижение не выражает, отвергается по имени.
///
/// Названная граница §10 вопроса 157, и она **шире** типовой стороны: `Flat`
/// выводится для семейства с тегом, а плотно уложить его понижение не умеет.
/// Отказ здесь стоит затем, чтобы расхождение двух перечней было названо, а не
/// проявилось чтением чужих байт.
#[test]
fn a_tagged_payload_is_refused_by_name() {
    let refused = harness::compiled(&format!("{SHAPE}{TAGGED}"))
        .expect_err("семейство с тегом плотной укладки не имеет (§10 вопрос 157)");
    let written = refused.to_string();
    assert!(
        written.contains("плоской укладки в понижении не имеет"),
        "отказ не называет причину - укладку: {written}"
    );
    // Ближайший проходящий сосед: тот же регион над примитивом.
    assert_eq!(
