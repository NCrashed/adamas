...
//! разница.
//!
//! **Абсолютные числа выросли на два с дроблением тел** (трек D волны 4), и это
//! названная цена, а не находка: кадр - блок кучи, и на каждый scope с ресурсом
//! их стало два - `CLOSING` и `CLOSED`, - потому что нормальный выход идёт
//! теперь трамплином, а не прямым вызовом деструктора. Несущее здесь -
//! **разности**: они не сдвинулись ни на одну.
//!
...
//! не достаётся ни тому, ни другому. Различающая программа пришла с живой
//! резумпцией - см. `general.rs`, где ветка держит резумпцию до своего конца.
...
fn the_destructor_runs_on_the_exceptional_path() {
    ...
    assert_eq!(
        empty, 15,
        "пустой деструктор на обрыве стоит не пятнадцать блоков"
    );
}
...
fn the_normal_exit_of_the_second_form_closes_its_frame() {
    ...
    assert_eq!(
        empty, 10,
        "пустой деструктор второй формы стоит не десять блоков"
    );
}
...
fn the_inner_resource_closes_first_when_the_body_aborts() {
    ...
    assert_eq!(note_last, 23, "`Note` связан последним: `flip` копирует");
}
