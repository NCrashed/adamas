...
//! остаётся верным. Сегодня его не покупают: `adamas check` целиком идёт 36 мс
//! на капстоуне в 944 строки, 20 мс на 772, 17 мс на 297 при бюджете
//! интерактивности порядка 100 мс. Граф зависимостей за такие деньги не берут,
//! и синхронный сервер на потоках здесь ровно к месту - отменять нечего.
//!
    ...
    for message in &connection.receiver {
        match message {
            ...
            Message::Notification(note) => {
                handle(connection, encoding, &mut documents, &note)?;
            }
