...
pub struct Crossing {
    ...
    /// Вариадическая ли функция, и сколько аргументов она объявляет поимённо.
    ///
    /// `None` - обычный прототип. `Some(n)` - `f(T0, …, Tn-1, ...)`: первые `n`
    /// **едущих** аргументов поимённые, остальные идут вариадической частью.
    /// Счёт именно едущих ([`Crossing::carried`]), а не связываний: единица и
    /// стёртое позиции у чужой стороны не занимают, а `...` стоит ровно про
    /// позиции.
    ///
    /// Различие это **обязано** доезжать до каждого потребителя и обязано быть
    /// точным: `SysV` AMD64 передаёт вариадической функции число использованных
    /// векторных регистров в `al`, и невариадический прототип его не ставит. То
    /// есть вызов вариадической функции через невариадический прототип (и
    /// наоборот) - неопределённое поведение, а не вольность записи.
