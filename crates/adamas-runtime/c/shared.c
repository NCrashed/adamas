 ...
 * создании региона». `SharedArena` и `SharedPool` расходятся между собой той же
 * одной строкой, что Arena и Pool, - тем, что делает `free`.
 *
 * Стандартных разделяемых стратегий §3.6 называет **две**, и третьей здесь нет
 * (`StackAlloc` есть размещение на C-стеке, а стек у каждого потока свой).
 * Операция LIFO при этом обслуживается наравне с прочими: семь операций у
 * области одни, и отказывать одной из них значило бы завести особый случай
 * там, где §3.6 его не называет. Вершина у неё **воркерская** - у общей
 * области вершины нет вовсе, - и пользовательская стратегия поверх этого
 * пишется обычным библиотечным кодом.
 *
