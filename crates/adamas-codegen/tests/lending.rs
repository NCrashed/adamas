...
//!
//! Чего здесь нет: машины. Массив у неё есть **спайн** `arrayNew`/`arraySet`
//! (`adamas_core::eval`), а не байты подряд, и адреса у него нет вовсе; отказ
//! назван (`RunError::ForeignBuffer`), свидетель ему - `adamas-interp`.
//!
...
fn source() -> String {
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/golden/programs/extern-buffer.adamas");
