...
/// Метод класса на примитиве: специализация отдаёт его **свёрнутым по эте**.
///
/// Измерено, а не предположено: `mono::specialise` заводит
/// `add@_,Add#Int64` с типом `Int64 -> Int64 -> Int64` и телом
/// `Const("Add#Int64.add")` - ведущих лямбд ноль. Понижение берёт параметры у
/// тела, поэтому арность выходит нулевая, а два аргумента идут применением к
/// значению - то есть через границу замыкания, где плоскому места нет.
///
/// Граница названа: развернуть определение до **объявленной** арности
/// понижение сегодня не берётся, и это правило про все определения, а не про
/// плоские, - решать его отдельно. Свидетель стоит здесь, чтобы следующая
/// сессия нашла его названным, а не переоткрыла.
const THROUGH_A_CLASS: &str = "\
type Int = Int64
...
instance Add Int64 where
  add x y = addInt64 x y

main : Int
main = add 1 2
";

/// Арифметика через класс отвергается названной причиной, а не считается молча.
#[test]
fn arithmetic_through_a_class_is_refused_by_name() {
    let error =
        harness::compiled(THROUGH_A_CLASS).expect_err("свёрнутый по эте метод не понижается");
    let text = error.to_string();
    assert!(
        text.contains("§4.11"),
        "отказ не назвал причину представлением: {text}"
    );
    // Ближайший проходящий сосед - тот же счёт без класса.
    assert_eq!(
        allocated(
            "сосед-класса",
            "\
