 ...
 * решение инлайнера.
 *
 * *Цена измерена, и она ноль.* Нагрузками из таблицы разрыва её мерить не на
 * чем - ни одна из пяти кадра не ставит вовсе (`adamas_kont_push` в их
 * порождённом C ноль вхождений), то есть вектора не трогает. Мерено на самой
 * кадроёмкой из доступных - хендлерном стенде `benches/native.rs`: парный
 * замер, пол девяти и одиннадцати чередующихся прогонов, **0.9926** и
 * **1.0014** против счётчика без ветви.
 *
