...
/// Последний сегмент квалифицированного имени: `Lib.Cmp.Bool` -> `Bool`.
///
/// Нужен там, где сверяется **имя-соглашение** (§4.3): объявление
/// подключаемого файла несёт путь, а соглашение знает короткое написанное имя.
/// Одна запись на всех, потому что расхождение в разборе точки было бы тихим.
#[must_use]
...
pub enum Term {
    ...
    /// контракт до вызова (§3.4). Правила вокруг неё - погашение, хендлеры,
    /// объявление эффектов - в [`crate::check`] и элаборации.
    ///
    /// **Индексы row отсчитываются из-под связывания**, как у кодомена:
    /// применение аргумент уже знает, и `(0 r : Region) -> {Alloc r} Nat`
    /// называет в эффекте именно тот регион, который передали. Применение
    /// поэтому инстанцирует row наравне с кодоменом
    /// ([`crate::value::RowClosure`]).
    ///
...
impl Term {
    ...
    pub fn mentions_recent(&self, depth: u32, window: u32) -> bool {
        ...
        match self {
            ...
            // Row стоит под связыванием стрелки наравне с кодоменом, поэтому и
            // считается на глубину больше.
