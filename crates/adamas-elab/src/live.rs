...
//! Так правило оказывается **точнее**, чем «встретилось имя», и остаток
//! неточности сужается до одного случая: имя стоит в позиции терма, но
//! попадает в `0`-параметр. Тогда `drop` не вставится, и ресурс утечёт.
//!
//! Направление ошибки одно и то же везде: лишний `drop` не вставляется
//! никогда - иначе отвергалась бы корректная программа, - а пропущенный
//! возможен только в названном случае.
//!
//! # Затенение
//!
