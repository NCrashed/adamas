...
pub(crate) struct Handler {
    ...
    pub multi: bool,
    /// Параметризован ли хендлер: `state s0` (§3.4, §10 вопрос 129).
    ///
    /// Ветка такого отвечает функцией от состояния и вправе позвать резумпцию
    /// **позже**, при применении ответа, - поэтому решение «резумпцию не
    /// позвали, продолжение мертво» на возврате ветки принимать нельзя. Оно
    /// откладывается кадром [`Frame::Settling`] вокруг этого применения.
    pub stateful: bool,
}
...
pub(crate) enum Frame {
    ...
    Branch(usize),
    /// Применение ответа параметризованной ветки к состоянию (§10 вопрос 129).
    ///
    /// Ветка `state`-хендлера отвечает функцией от состояния и зовёт резумпцию
    /// при её применении, поэтому на возврате ветки решать «продолжение
    /// мертво» нельзя - кадр откладывает решение до конца этого применения.
    /// Ставит его [`Machine::settled`], перехватывая ожидающее применение
    /// состояния, которое по построению формы стоит под кадром ветки.
    Settling(usize),
...
impl Segment {
    ...
    /// Тот же сегмент без нижнего кадра.
    ///
    /// Нужен отмене одной задачи: сегмент уступившего файбера включает **его
    /// собственный** кадр питомника - им возобновление ставит файбер обратно
    /// под свой круг (`parking`). Раскрутке этот кадр не нужен, а прочитанный
    /// ею означает «питомник брошен» и вычерпывает круг целиком - отмена одной
    /// задачи сносила остальные (ревью 2026-09-08).
    ///
    /// Снимается ровно один кадр, самый нижний: питомник **внутри** сегмента -
    /// тот, что файбер открыл сам, - брошен по-настоящему, и раскрутка обязана
    /// его пройти.
    pub(crate) fn without_base(&self) -> Self {
        let mut links: Vec<Link> = (*self.links).clone();
        let Some(first) = links.first_mut() else {
            return self.clone();
        };
        if first.is_empty() {
            return self.clone();
        ...
        first.remove(0);
        // Опустевшее звено убирается: `first` читает нижний кадр через первое
        // звено, и пустое сделало бы его невидимым.
        if first.is_empty() {
            links.remove(0);
        }
        Self {
...
impl Kont {
    ...
    /// Снимает вершину, только если это кадр применения-аргумента.
    ///
    /// Нужно ровно одному месту - отложенному решению параметризованного
    /// хендлера: под кадром ветки по построению формы стоит ожидающее
    /// применение состояния, и решение о смерти резумпции заводится вокруг
    /// него (§10 вопрос 129). Всякий другой кадр остаётся на месте.
    pub(crate) fn pop_argument(&mut self) -> Option<Frame> {
        let link = self.links.last_mut()?;
        if matches!(link.last(), Some(Frame::Argument(..))) {
            return self.pop();
        }
        None
    }
