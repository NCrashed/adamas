        ...
        match expr {
            ...
            Expr::Foreign {
                ...
                let called = if held.is_empty() {
                    ...
                    // Представление ответа записывается в факт, и записывается
                    // обязательно: по нему эмиттер объявляет временное. У
                    // `Fact::opaque` оно указательное, а ответом чужой функции
                    // бывает плоское слово - и `adamas_value`, в которое его
                    // положили бы, сборки не переживёт (мутант: 2 красных).
                    //
                    // В [`Pass::flat`] этот номер при этом **не** кладётся, и
                    // это не забывчивость: узел ниже собран уже обработанным,
                    // через [`Pass::expr`] он не пойдёт, а `flat` читает только
