...
fn declare_data(
    ...
    // Поле конструктора получает `ω`, а §4.1 назначает ему `1`: расхождение
    // сознательное и разобрано §10 вопросом 65. Коротко - поле из разбора не
    // масштабируется тем, как расходуется разбираемое, и `1` по умолчанию
    // отвергает `plus (Succ k) m = Succ (plus k m)`.
