...
pub enum Cross {
    ...
    /// Колбэк уровня 1: адрес нашего определения, видимого C (§5.3).
    ///
    /// Едет **слово**, и форма самого колбэка сюда не кладётся. Довод от
    /// уровня: прототип чужой функции печатается по [`Crossing::carried`], то
    /// есть списком `PrimTy`, и указателя на функцию там не выразить; а ABI
    /// System V кладёт адрес в тот же класс регистров, что и целое, поэтому
    /// `uint64_t` в этой позиции есть верный вызов, а не приближение. Названная
    /// цена: прототип теряет форму колбэка, и разойдись она с тем, чего ждёт
    /// чужая сторона, - не поймает никто. Это ровно граница уровня 1, уже
    /// названная §10 вопросом 183, а не новая дыра.
    Callback,
}
...
impl Cross {
    ...
    pub const fn carried(self) -> Option<crate::prim::PrimTy> {
        match self {
            ...
            Self::Buffer(_) | Self::Callback => Some(crate::prim::PrimTy::UInt64),
        }
    }
