...
impl Metas {
    ...
    pub fn release(&mut self) {
        ...
        //
        // Перебирают его `settle` и `check::settle_terms`, а зовутся они из
        // проверки написанного типа и из объявления - но не из проверки
        // **тела**. Сегодня тело ничего не откладывает, и инвариант держится
        // сам; охраны у него не было, хотя у соседнего - живого диапазона
        // слотов - она громкая. Жанр тот же: новое состояние заведено рядом
        // со старым, а проверка старого на него не распространилась.
        assert!(
