...
/// зависит; почему именно так - см. [`the_sign_of_a_computed_nan_is_not_ours`].
const ORDER: &str = "\
data Bool where
  True : Bool
  False : Bool
...
/// Программа, наблюдающая **знак** вычисленного NaN.
///
/// В договор вычислителей не входит и входить не может - см.
/// [`the_sign_of_a_computed_nan_is_not_ours`], который ею и меряет.
const NAN_SIGN: &str = "\
data Bool where
  True : Bool
  False : Bool
...
main : Int64
main = pick (ltFloat64 undefined 0.0) 1 0
";
...
/// Знак вычисленного NaN нам не принадлежит, и `totalOrder` его показывает.
///
/// Найдено этим треком, замером, а не рассуждением. IEEE-754 знак и полезную
/// нагрузку NaN, порождённого недопустимой операцией, **не специфицирует**, и
/// стороны расходятся: железо x86 отдаёт `0xFFF8000000000000` (знак стоит),
/// свёртка констант LLVM - `0x7FF8000000000000` (знака нет). Пока NaN
/// печатается как `NaN`, разница не видна; `totalOrder` §4.3 упорядочивает NaN
/// **знаком**, и разница становится ответом программы.
///
/// Тест не чинит - правкой эмиттера это не чинится. Он **закрепляет дыру
/// прогоном**, тем же приёмом, каким соседний закрепляет границу обещания:
/// расхождение здесь ожидаемый исход, а зелёное молчание было бы обманчивым
/// свидетелем. Сойдись однажды все стороны - тест упадёт, и это правильный
/// сигнал: дыра закрылась, запись в §10 и этого свидетеля пора переписать.
///
/// Измерено 2026-09-15: машина `1`, C `1` на всех четырёх уровнях, LLVM `1` на
/// `-O0` и `0` на `-O1`, `-O2`, `-O3`. Порог - свёртка констант в `opt`.
#[test]
fn the_sign_of_a_computed_nan_is_not_ours() {
    let mut seen = vec![("машина".to_owned(), by_machine(NAN_SIGN))];
    for level in LEVELS {
        seen.push((
            format!("C {level}"),
    ...
    let Some((tools, _)) = harness::llvm_toolchains() else {
        eprintln!("знак вычисленного NaN без LLVM: {}", row(&seen));
        return;
    };
    for level in LEVELS {
        seen.push((
    ...
    eprintln!("знак вычисленного NaN: {}", row(&seen));
    let first = &seen[0].1;
    assert!(
        seen.iter().any(|(_, answer)| answer != first),
        "все стороны сошлись на знаке вычисленного NaN: дыра закрылась, \
         запись §10 и этот свидетель устарели ({})",
        row(&seen)
    );
}
