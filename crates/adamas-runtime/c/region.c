 ...
 *
 * Делить приходится самим: gcc разбивает такую функцию частичным инлайнингом
 * сам (`.part.0`), а конвейер LLVM нет - `PartialInlinerPass` в его `-O2`
 * выключен по умолчанию. На `adamas_array_writable` это стоило 72% строки 4а
 * таблицы разрыва (трек B волны 3 Фазы 7).
 *
 * **Написать две функции при этом не значит разделить**, и это измерено, а не
 * предположено (`docs/measurements/region-alloc/`): без атрибута компилятор
 * втягивает `copied` обратно, и две функции идут вровень со слитой - 1.01,
 * 1.09, 0.98. Делит атрибут, и цена его на LLVM-пути **1.459** раза, у gcc
 * 1.120. `cold` здесь не украшение к `noinline`: он метит место вызова
