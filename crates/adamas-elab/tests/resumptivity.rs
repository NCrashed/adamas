...
///
/// Каждая метка здесь - отдельная форма ветки, и вердикт у неё свой. Перечень
/// стоит в шапке [`adamas_core::resume`]; здесь его половина, написуемая на
/// поверхностном языке, а позиции с `let` и резумпцию значением проверяют
/// термами unit-тесты того же модуля - тело ветки язык берёт одним выражением,
/// и `let` в нём сегодня не пишется. Разойдись перечень со свидетелями, правило
/// оказалось бы закрыто на том случае, на котором его нашли.
#[test]
