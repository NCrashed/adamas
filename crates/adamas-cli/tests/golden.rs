...
/// Проект из десяти файлов проверяется и считается (§4.8, §7.2).
///
/// Фикстура одна, а файлов в ней десять: библиотека разложена по девяти
/// модулям `Std/*`, подключающим друг друга, а во входном файле нет ни одного
/// имени счёта - все приходят из других файлов. Сломай разрешение имён между
/// файлами, и проверка ляжет целиком; ответ при этом позиционный, из 22 чисел,
/// и перестановка его не переживает.
///
/// Он же - предмет замера полной перепроверки
/// (`docs/measurements/project-recheck/`): цену интерактивности называют на
/// проекте, который существует, а не на десяти копиях одного файла.
#[test]
fn a_project_of_ten_files_checks_and_evaluates() {
    let entry = corpus().join("project").join("main.adamas");
    let (passed, text) = checked(&entry);
    ...
    assert!(passed, "проект не вычислился:\n{text}");
    insta::assert_snapshot!("project-main", text);
}
