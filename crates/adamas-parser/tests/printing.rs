...
fn a_long_application_spine_prints_without_recursion() {
    // Аргументы разбор набирает циклом, предел вложенности на них не тратится:
    // спайн бывает глубже любого дерева, собранного скобками, и рекурсия по
    // нему роняла печать на нескольких тысячах аргументов.
    // Длина - с запасом к тому, на чём печать падала (около трёх тысяч), и в
    // пределах того, что переживает освобождение дерева: `Drop` у цепочки
    // `Box` тоже рекурсивен, и на десятках тысяч упирается в стек уже он.
    let text = format!("f = g{}\n", " x".repeat(10_000));
