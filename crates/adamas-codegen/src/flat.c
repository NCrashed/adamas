...
/* Дескриптор укладки (§4.11): `Layout` есть `{ size : UInt32, align : UInt32 }`,
 * и в рантайме он - два слова по значению.
 *
 * Живёт он **здесь**, а не в `adamas.h`: рантайм его не читает вовсе. Шаг
 * индексации массив носит в собственном заголовке, а кто этот шаг посчитал -
 * тип элемента или дескриптор из контекста, - решает порождённый код. Словарь
 * `Flat a` в понижении есть ровно эта пара: метод у класса один, и метод этот
 * `layout`. */
typedef struct adamas_layout {
    uint32_t size;
    uint32_t align;
} adamas_layout;
