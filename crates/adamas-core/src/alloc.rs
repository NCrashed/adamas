...
//! на линейном или unique входе. Вердикт считает такой конструктор
//! аллоцирующим всегда, и причина **не** в том, что механизма нет: оба
//! понижения вставляют `adamas_reuse` (`codegen::perceus`, с 2026-09-09).
//! Причина в том, что срабатывание решается **в рантайме** - счётчиком ссылок
//! в точке разбора, - а `@noalloc` есть статическое обещание, и опереть его на
//! динамическое условие нечем. Направление консервативно: отвергается часть
//! законных определений, но не принимается ни одно аллоцирующее.
//!
