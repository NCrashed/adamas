...
/// параметрам. Иначе параметры связаны между собой, и такое определение
/// отвергается с указанием на связь, а не принимается с множеством, которое
/// обещало бы больше законных подстановок, чем есть.
///
/// Сама связь чаще всего выражается: у композиции домен равен `q · r`, и
/// произведение в позиции кратности пишется - `(q * r z : a)`. Написанное, оно
/// сворачивается здесь же, подстановкой, и множество снова становится
/// произведением.
fn check_graded(
    signature: &Signature,
    metas: &mut Metas,
        ...
        Term::Const(head_name, levels, _) => {
            head_name == data
                ...
                && levels
                    .iter()
                    .zip(expected.iter())
                    // Дырка на месте параметра означает семейство, чьё
                    // обобщение отложено до конструкторов (§10 вопрос 53):
                    // параметров у него ещё нет, аргументы ссылки инертны, и
                    // сверять их с `u0` рано - перепишет их само обобщение.
                    // На обычном пути дырка сюда не доходит: её уже заменил
                    // параметр либо решение.
                    .all(|(found, wanted)| found.equiv(wanted) || matches!(found, Level::Meta(_)))
        }
        _ => false,
    };
...
/// Универсум, которого хватит полям конструктора (§10 вопрос 109).
///
/// Семейство обязано вместить свои поля: иначе `Type ℓ` содержал бы значение,
/// построенное над `Type (ℓ+1)`, и предикативность (§3.2) обходилась бы через
/// data-декларацию. Параметры это правило не ограничивает - они не хранятся в
/// значении, - и поэтому первые `params` полей пропускаются.
///
/// Считается **после** фазы B1: до конструкторов знать это неоткуда, а
/// написать автору нечем - уровень в kind'е не пишется (§3.2). Раньше сорт
/// заземлялся нулём и поднимался только по параметрам, поэтому конструктор,
/// поднявший свободную типовую переменную, отвергался всегда: поле `{0 a :
/// Type ℓ}` живёт в `Type (ℓ+1)`.
///
/// Рекурсивное поле круга не даёт: его уровень и есть текущий сорт, а `max` от
/// себя же идемпотентен.
///
/// # Errors
///
/// Поле, не являющееся типом.
pub fn constructor_sort(
    signature: &Signature,
    metas: &mut Metas,
    params: u32,
    ty: &Term,
...
fn infer_app(
    ...
    // ничего не стоят".
    //
    // Аргумент проверяется **до** погашения, и порядок этот существенный.
    // Row вызываемого бывает дыркой, которую определяет как раз аргумент:
    // у `#mask.L` она есть row маскируемого вычисления. Погашение, спрошенное
    // раньше, склеивает её с хвостом окружающей - а потом аргумент заставляет
...
///
/// Правило: `ε' ≡ ε ++ Λ`. Λ определяется однозначно - порядок между разными
/// именами несуществен, а внутри имени значим, - поэтому метки вызываемого суть
/// **префикс** меток окружающей, а Λ есть их разность.
///
/// Хвост Λ носит ровно тогда, когда row вызываемого **замкнута**: тогда весь
/// остаток окружающей, включая её хвост, и есть Λ. У открытой row вызываемого
/// хвост общий с окружающей, и Λ замкнута.
///
/// Префикс, а не суффикс, и это не деталь: `{State Int} ⊑ {State Int, State
/// Int}` отдаёт вызываемому **внутреннее** вхождение. Хендлер кладёт свою метку
/// в начало окружающей, а вызываемый вызван под ним - значит метка стоит прямо
/// перед ним, и она же ему достаётся.
///
/// Следствие важнее удобства: Λ есть то, что **за** row вызываемого, поэтому
/// смещение вектора evidence всегда нуль, и записывать его в терм не нужно
/// вовсе. Обратная ориентация требовала бы `|Λ|` на каждом вызове операции.
/// Цена названа: хендлеры динамически scoped - вызываемый, написанный до
/// внутреннего хендлера, будет им перехвачен.
fn discharges(
    sig: &Signature,
    metas: &mut Metas,
    size: u32,
    ...
    // Обе row разворачиваются по решённым хвостам: решением хвоста бывает row
    // с метками, а `zonk_tail` таких не разворачивает - его тип один хвост.
    ...
    // решается здесь - это и есть унификация scoped labels (§3.4): метки
    // сопоставлены циклом ниже, остаток уходит в хвостовую метапеременную.
    //
    // **Замкнутая row вызываемого хвоста не требует** (§10 вопрос 119). Она не
    // утверждает, что окружение пусто, - она говорит, что вызываемый производит
    // только это; хвост окружающей уходит в Λ вместе с лишними метками.
    // Однозначность Λ от этого не страдает: держится она на **префиксности**
    // меток, которую проверяет цикл ниже, а не на отсутствии хвоста. Смещение
    // вектора evidence остаётся нулём по той же причине - Λ стоит **за** row
    // вызываемого, и сколько её и есть ли у неё хвост, ему безразлично.
    //
    // Без этого случая замкнутая непустая row была недостижима под открытой
    // окружающей, а открыта она у всякого определения: определение, позвавшее
    // эффектную функцию **из поля конструктора**, само становилось замкнутым и
    // не звалось ниоткуда. Колбэк в структуре данных - таблица обработчиков,
    // список подписчиков, очередь файберов - не писался вовсе. Пустая row -
    // наименьший её случай, а не отдельное исключение: меток нет, цикл ниже
    // пуст, и она проходит тем же путём.
    if wanted.tail().is_some() && !metas.unify_tails(ambient.tail(), wanted.tail()) {
        return false;
    }
