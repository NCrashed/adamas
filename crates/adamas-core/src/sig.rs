...
pub struct Signature {
    ...
    unit: Option<Name>,
    /// Чужие символы: имя определения - имя у линкера (§5.3).
    foreign: HashMap<Name, Name>,
}
...
impl Signature {
    ...
    /// Имя чужого символа у линкера. `None` - определение не объявлено
    /// `extern` (§5.3).
    ///
    ...
    #[must_use]
    pub fn foreign(&self, name: &str) -> Option<&Name> {
        self.foreign.get(name)
    }

    /// Записывает, каким символом определение зовётся у линкера (§5.3).
    pub fn name_foreign(&mut self, declared: &Name, symbol: &Name) {
        self.foreign.insert(Rc::clone(declared), Rc::clone(symbol));
    }
